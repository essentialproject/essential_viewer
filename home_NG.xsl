...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="allHomePortals" select="$ngPortal//simple_instance"/>
	<xsl:variable name="allPortalsSections" select="$ngPortal_Section//simple_instance"/>
	<xsl:variable name="allPortalPanels" select="$ngPortal_Panel//simple_instance"/>
	<xsl:variable name="allEnabledPortals" select="$allHomePortals[own_slot_value[slot_reference = 'portal_is_enabled']/value = 'true']"/>
	...
	<xsl:key name="portalSection_key" match="$allPortalsSections" use="own_slot_value[slot_reference = 'parent_portal']/value"/>
	<xsl:key name="portalPanel_key" match="$allPortalPanels" use="own_slot_value[slot_reference = 'panel_for_portal']/value"/>
	...
	<xsl:variable name="allHomeReports" select="$ngReport//simple_instance"/>
	<xsl:variable name="currentPortal" select="$allHomePortals[own_slot_value[slot_reference = 'name']/value = 'Essential Viewer Master Portal']"/>
	...
	-->
	<xsl:variable name="portalSections" select="key('portalSection_key',$currentPortal/name)"/>
	<xsl:variable name="portalPanels" select="key('portalPanel_key',$currentPortal/name)"/> 
				...
				<div class="container-fluid">
					<div class="row">
						<div class="col-xs-12 col-sm-8">
							<div class="row">
								<xsl:apply-templates mode="portals" select="$allHomePortals">
	...
	<xsl:template name="viewLibrary">
		<xsl:variable name="viewLibraryReport" select="$allHomeReports[own_slot_value[slot_reference = 'name']/value = 'Core: View Library']"/>
	...
	<!--		<xsl:variable name="portalPanelSections" select="$allPortalPanelSections[name = current()/own_slot_value[slot_reference = 'portal_panel_sections']/value]"/>-->
			...
			<xsl:variable name="panelReports" select="$allHomeReports[name = current()/own_slot_value[slot_reference = 'reports_for_portal_panel']/value]"/>
	...
	<xsl:template mode="portalPanelSections" match="node()">
		<xsl:variable name="this" select="current()"/>
		<xsl:if test="eas:isUserAuthZ($this)">
			<xsl:variable name="sectionReports" select="$allHomeReports[name = current()/own_slot_value[slot_reference = 'portal_panel_section_reports']/value]"/>
