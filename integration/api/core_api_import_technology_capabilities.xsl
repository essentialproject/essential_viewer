...
<xsl:template mode="techCapabilities" match="node()">
       <xsl:variable name="parent" select="$techDomain[name=current()/own_slot_value[slot_reference='belongs_to_technology_domain']/value]"/>
        ...
        "domain":"<xsl:call-template name="RenderMultiLangInstanceName"><xsl:with-param name="theSubjectInstance" select="$parent"/><xsl:with-param name="isForJSONAPI" select="true()"/></xsl:call-template>"
