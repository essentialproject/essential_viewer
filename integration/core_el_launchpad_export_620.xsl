                     ...
                     <div class="tab-content">
                        ...
                        <div id="launchpadplus" class="tab-pane fade">
                           ...
                           <br/>
                              ...
                              <button class="btn btn-success" id="exportStrategicPlans">Strategic Plans</button>
                              <button class="btn btn-success" id="exportApplicationIntegrations">Application Integrations</button>
                              <button class="btn btn-success" id="exportControlFramework">Control Frameworks</button>
                              <button class="btn btn-success" id="exportStratPlanner">Strategy Planner</button>
                              <button class="btn btn-success" id="exportTechRef">Technology Reference</button>
                              <button class="btn btn-success" id="exportTechnologyLifecycles">Technology Lifecycles</button> 
                              <button class="btn btn-default" id="exportSupplierContractManagement">Supplier Contract Management</button> 
                              <button class="btn btn-default" id="exportApplicationKPIs">Application KPIs</button> 
                              <button class="btn btn-default" id="exportBusinessModels">Business Models</button>
                              <button class="btn btn-default" id="exportValueStreams">Value Streams</button>  
...
Handlebars.registerHelper('greaterThan', function (v1, v2, options) {
   ...
   <xsl:template name="RenderViewerAPIJSFunction">
      ...
      var allProducts = [<xsl:apply-templates select="$products" mode="dataLookup"/>]
      ...
      var allprojlifecycle = [<xsl:apply-templates select="$projlifecycle" mode="dataLookup"/>]
		...
		var allPlanningAction=[<xsl:apply-templates select="$planningAction" mode="dataLookup"/>]
      ...
      var custEx=[<xsl:apply-templates select="$custEx" mode="standard"/>]
      var custEmotions=[<xsl:apply-templates select="$custEmotions" mode="standard"/>]
      var custServiceQual=[<xsl:apply-templates select="$custServiceQual" mode="dataLookup"/>]
      var custServiceQualVal=[<xsl:apply-templates select="$custServiceQualVal" mode="valLookup"/>]
      var custJourney=[<xsl:apply-templates select="$custJourney" mode="custJourney"/>]
      var custJourneyPhase=[<xsl:apply-templates select="$custJourneyPhase" mode="custJourneyPhase"/>]
...
"fromApptype":"<xsl:value-of select="$fromApp/type"/>",
"toAppType":"<xsl:value-of select="$toApp/type"/>",
...
"info2":	[<xsl:for-each select="$allInfoEdges[type='APP_PRO_TO_INFOREP_EXCHANGE_RELATION']">
   <xsl:variable name="thisInfoReps" select="key('allInfoRepKey',current()/own_slot_value[slot_reference='app_pro_to_inforep_to_inforep']/value)"/>
   <xsl:variable name="thisInfoRepsExchanged" select="key('allAppProtoInfoKey',current()/own_slot_value[slot_reference='atire_app_pro_to_inforep']/value)"/>
   <xsl:variable name="thisInfoReps2" select="key('allInfoRepKey',$thisInfoRepsExchanged/own_slot_value[slot_reference='app_pro_to_inforep_to_inforep']/value)"/>
   <xsl:variable name="aq" select="key('aquisitionVal',current()/own_slot_value[slot_reference='atire_acquisition_method']/value)"/>
   <xsl:variable name="fq" select="key('infoQualityVal', current()/own_slot_value[slot_reference='atire_service_quals']/value)"/>
	{
		"id":"<xsl:value-of select="current()/name"/>",
		<xsl:variable name="infoTemp" as="map(*)" select="map{'name': string(translate(translate($thisInfoReps2/own_slot_value[slot_reference = ('name')]/value,'}',')'),'{',')'))}"></xsl:variable>
      ...
      <xsl:variable name="aqTemp" as="map(*)" select="map{'aquisition': string(translate(translate($aq/own_slot_value[slot_reference = ('name')]/value,'}',')'),'{',')'))}"></xsl:variable>
		<xsl:variable name="aqResult" select="serialize($aqTemp, map{'method':'json', 'indent':true()})"/>  
		<xsl:value-of select="substring-before(substring-after($aqResult,'{'),'}')"></xsl:value-of>,
      <xsl:variable name="fqTemp" as="map(*)" select="map{'frequency': string(translate(translate($fq/own_slot_value[slot_reference = ('name')]/value,'}',')'),'{',')'))}"></xsl:variable>
		<xsl:variable name="fqResult" select="serialize($fqTemp, map{'method':'json', 'indent':true()})"/>  
		<xsl:value-of select="substring-before(substring-after($fqResult,'{'),'}')"></xsl:value-of> 
	}<xsl:if test="position()!=last()">,</xsl:if>
</xsl:for-each>],
<!-- allInfoExchange
aquisitionVal
