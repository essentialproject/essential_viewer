...
<xsl:stylesheet xpath-default-namespace="http://protege.stanford.edu/xml" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:functx="http://www.functx.com" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<!-- 20.01.2012	JWC	Updated for Application Deficiency / Redundancy -->
	...
	<xsl:variable name="linkClasses" select="('Technology_Component', 'Technology_Component')"/>
	...
	<xsl:variable name="gapReportLabel">
		<xsl:value-of select="eas:i18n('Technology Change Analysis - Technology Component')"/>
	</xsl:variable>

	<!-- CHANGE THIS TEXT TO LIST THE TYPES OF ELEMENTS IN SCOPE FOR THE Change Analysis (USED IN DESCRIPTIONS) -->
	<xsl:variable name="gapReportElementDesc" select="'Technology Components'"/>
	...
	<xsl:template name="PrintElementTypePrefix">
		...
		<xsl:choose>
			...
			<xsl:when test="$elementType = 'Technology_Component'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Technology Composite')"/>
				</span>
				<br/>
			</xsl:when>
		</xsl:choose>
	...
	<xsl:template name="PrintElementTypeSuffix">
		...
		<xsl:choose>
			...
			<xsl:when test="$elementType = 'Technology_Composite'">
				<br/>
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Technology Composite')"/>
				</span>
	...
	<xsl:template name="PrintElementDisplayLabel">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Technology_Component'">
				<!--<xsl:variable name="aTechCompName" select="$theElement/own_slot_value[slot_reference='name']/value" />
				<a id="{$aTechCompName}" class="context-menu-techComponent menu-1 text-white">
					<xsl:call-template name="RenderLinkHref">
						<xsl:with-param name="theInstanceID" select="$theElement/name" />
						<xsl:with-param name="theXML" select="$reposXML" />
						<xsl:with-param name="theParam4" select="$param4" />
						<!-\- pass the id of the taxonomy term used for scoping as parameter 4-\->
					</xsl:call-template>
					<xsl:value-of select="$aTechCompName" />
				</a>-->
	...
	<xsl:template name="BuildPage">
		...
		<xsl:variable name="allGapElements" select="/node()/simple_instance[(type = 'Technology_Component') or (type = 'Technology_Composite')]"/>
