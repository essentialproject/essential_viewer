...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="prodDeploymentRoleId" select="/node()/simple_instance[type='Deployment_Role'][own_slot_value[slot_reference = 'name']/value = 'Production']/name"/>
		...
		<html>
			<head>
				...
				<script>
					...
					const prodAppDepRoleId = "<xsl:value-of select="$prodDeploymentRoleId"/>";

					class AppDeploymentTechPlatform {
						constructor(appName, depRoleId, techProdRoles) {
							this.name = appName + " - Production";
							this.environmentType = {
								"id": depRoleId
							}
							let tprList = [];
							techProdRoles.forEach(tpr => {
								tprList.push(
									{
										"techProductAsType": {
											"id": tpr.id
							...
							this.technologyPlatform = {
								"name": appName + " - Production Technology Platform",
        						"containedTechProducts": tprList
							};
						}
					}

					class AppDeploymentTechPlatformDeps {
						constructor(appDep, tpruDepsList) {
							this.id = appDep.id;
							this.technologyPlatform = {
								"id": appDep?.technologyPlatform?.id,
								"techProductDependencies": tpruDepsList
							};
						}
					}
					...
					function renderAppDeploymentTechPlatformJSON() {
						<!-- console.log('APP TECH PLATFORM INPUTS:', {
							"appDepRoleId": prodAppDepRoleId,
							"selectedApp": selectedApp,
							"selectedTechProds": selectedTechProds
						}); -->
						let appTechPlatformJSON;
						if(prodAppDepRoleId.length > 0 &amp;&amp; selectedApp &amp;&amp; selectedTechProds?.length > 0) {
							let appName = selectedApp.name; 
							appTechPlatformJSON = new AppDeploymentTechPlatform(appName, prodAppDepRoleId, selectedTechProds);
						}
						console.log('APP TECH PLATFORM JSON:', appTechPlatformJSON);
						return appTechPlatformJSON;
					}

					function renderAppTechArchDepsJSON(tpruIdMap) {
						let usedTechProdRoleIds = [];
						let dependencyRels = [];

						let aSelectedRefArch, thisRefArchRelations, aRefArchRelation, fromTechProdRole, toTechProdRole;
						let fromTechProdRoleId, toTechProdRoleId, fromTPRUsageId, toTPRUsageId;
						...
						for (var i = 0; thisSelectedRefArchs.length > i; i += 1) {
							...
							for (var j = 0; thisRefArchRelations.length > j; j += 1) {
								...
								if(fromTechProdRole != null) {
									...
									fromTPRUsageId = tpruIdMap[fromTechProdRoleId];
								} else  {
									fromTechProdRoleId = '';
									fromTPRUsageId = null;
								}
								...
								if(toTechProdRole != null) {
									...
									toTPRUsageId = tpruIdMap[toTechProdRoleId];
								} else  {
									toTechProdRoleId = '';
									toTPRUsageId = null;
								}
								...
								if((fromTPRUsageId != null) || (toTPRUsageId != null)) {
									...
									//add a new dependency relation
									dependencyRels.push({
										"sourceId": fromTPRUsageId,
										"targetId": toTPRUsageId
									});
								}
							}
						}
						return dependencyRels;
					}


					function updateAppDepTechDependencies(appDep) {
						let appDepTPRUs = appDep?.technologyPlatform?.containedTechProducts;
						let tpruDict = {};
						appDepTPRUs?.forEach(tpru => {
							let idKey = tpru.techProductAsType.technologyType.id + tpru.techProductAsType.technologyProduct.id;
							tpruDict[idKey] = tpru.id;
						});
						if(appDepTPRUs?.length > 0) {
							let tpruDepsList = renderAppTechArchDepsJSON(tpruDict);
							let appTechPlatformDepsData = new AppDeploymentTechPlatformDeps(appDep, tpruDepsList);

							let resourcePath = 'applications/' + selectedApp.id + '/deployments';
							essPromise_updateAPIElement('/essential-core/v1', appTechPlatformDepsData, resourcePath, 'Application Deployment')
							.then(function(response) {
								selectedApp.prodDeployment = response;
								$('.app-tech-platform-api').hide();
							})
							.catch(function(error) {
								let errorMessage = 'An error occurred when creating the Application Technology Platform product dependencies';
								console.log(errorMessage, error);
								$('#app-tech-platform-api-error').text(errorMessage);
							});
						} else {
							console.log('No Technolog Platform product dependencies defined');
							$('.app-tech-platform-api').hide();
						}
					}

					function createAppDeployment(appDepData) {
						$('#app-tech-platform-api-error').text('');
						let resourcePath = 'applications/' + selectedApp.id + '/deployments';
						essPromise_createAPIElement('/essential-core/v1', appDepData, resourcePath, 'Application Deployment')
						.then(function(response) {
							selectedApp.prodDeployment = response;
							updateAppDepTechDependencies(response);
						})
						.catch(function(error) {
							let errorMessage = 'An error occurred when creating the Application Technology Platform';
							console.log(errorMessage, error);
							$('#app-tech-platform-api-error').text(errorMessage);
						});
					}
					...
					function renderAppTechArchExportData() {
						let dataTableSet = [];
						let usedTechProdRoleIds = [];
						let usedAPITechProdRels = [];
						let dataTableRow, appLink, aSelectedRefArch, thisRefArchRelations, aRefArchRelation, fromTechProdRole, toTechProdRole;
						let fromTechProdRoleId, toTechProdRoleId;
					...
					//function to create the data structure needed to render the application technology architecture API data
					function renderAppTechArchAPIData() {
						console.log('SELECTED REF ARCHS', thisSelectedRefArchs);
						console.log('SELECTED TECH PRODS', selectedTechProds);
						...
						for (var i = 0; thisSelectedRefArchs.length > i; i += 1) {
							...
							for (var j = 0; thisRefArchRelations.length > j; j += 1) {
								...
								if((fromTechProdRole != null) || (toTechProdRole != null)) {
									//add a relation object to the api relation list
									usedAPITechProdRels.push({
										"sourceId": fromTechProdRole.id,
										"targetId": toTechProdRole.id
									});
					...
					function addSelectedTechProduct(productId) {
						...
						if((selectedTechArch != null) &amp;&amp; (selectedTechComp != null) &amp;&amp; (productId != null)) {
							...
							//console.log('ADDED TECH PROD ROLE:', selectedTechProd);

							if(existingSelectedTechProd != null) {
								existingSelectedTechProd.id = existingSelectedTechProd.id,
					...
					$(document).ready(function(){
						...
						<!-- FUNCTIONS TO CREATE THE APP TECH PLATFORM IN THE REPOSITORY -->
						$('#create-app-tech-platform-btn').on('click', function(evt) {
							let appTechPlatformJSON = renderAppDeploymentTechPlatformJSON();
							if(appTechPlatformJSON) {
								createAppDeployment(appTechPlatformJSON);
							}
						});
	...
	<xsl:template name="RenderResultsPanel">
		<h3 class="section-title bg-black">i. Selected Technology Products</h3>
			...
			<xsl:if test="$eipMode">
				<h3 class="section-title bg-black app-tech-platform-api">iii. Create Application Technology Architecture</h3>
				<div class="dashboardPanel bg-white app-tech-platform-api">
					<p>Click the button below to confirm the selected Technology Products that comprise the solution's Technology Platform.</p>
					<button id="create-app-tech-platform-btn" type="button" class="btn btn-success">Create Technology Platform</button>
					<span id="app-tech-platform-api-error" class="left-20 textColourRed"></span>
				</div>
			</xsl:if>
	</xsl:template>
