...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="linkClasses" select="('Technology_Node')"/>
	<!-- END GENERIC CATALOGUE SETUP VARIABES -->

	<xsl:variable name="pageLabel">
		<xsl:value-of select="eas:i18n('Technology Node Catalogue by Name')"/>
	</xsl:variable>
				...
				<div class="container-fluid">
					<div class="row">
						<xsl:variable name="view_url">
							<xsl:text>report?XML=reportXML.xml&amp;XSL=technology/core_tech_node_list_by_vendor.xsl&amp;PMA=&amp;LABEL=Physical Node Catalogue By Vendor</xsl:text>
						</xsl:variable>
						<div class="col-xs-12">
					...
					<div class="row">
						<div class="col-xs-12">
							...
							<p><xsl:value-of select="eas:i18n('Click on one of the Physical Nodes below to navigate to the required view')"/>.</p>
	...
	<xsl:template name="Index">
		...
		<xsl:choose>
			<xsl:when test="string-length($viewScopeTermIds) > 0">
				<xsl:variable name="nodeList" select="$allTechNodes[(own_slot_value[slot_reference = 'element_classified_by']/value = $viewScopeTerms/name) and ((starts-with(own_slot_value[slot_reference = 'name']/value, $letterCap)) or (starts-with(own_slot_value[slot_reference = 'name']/value, $letterLow)))]"/>
				<div class="tech_nodes">
					<ul>
						<xsl:apply-templates select="$nodeList" mode="PhysicalNode">
			...
			<xsl:otherwise>
				<xsl:variable name="nodeList" select="$allTechNodes[((starts-with(own_slot_value[slot_reference = 'name']/value, $letterCap)) or (starts-with(own_slot_value[slot_reference = 'name']/value, $letterLow)))]"/>
