...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<!-- 20.01.2012	JWC	Constructed the Technology Distribution Change Analysis -->
	...
	<xsl:variable name="linkClasses" select="('Business_Process', 'Business_Activity', 'Business_Task', 'Business_Rule', 'Technology_Node', 'Hardware_Instance', 'Infrastructure_Software_Instance', 'Application_Software_Instance', 'Information_Store_Instance', 'Technology_Deployment_Group')"/>
	...
	<xsl:variable name="gapReportLabel">
		<xsl:value-of select="eas:i18n('Technology Change Analysis - Technology Distribution')"/>
	</xsl:variable>

	<!-- CHANGE THIS TEXT TO LIST THE TYPES OF ELEMENTS IN SCOPE FOR THE Change Analysis (USED IN DESCRIPTIONS) -->
	<xsl:variable name="gapReportElementDesc" select="'Technology Instances or Nodes'"/>
	...
	<xsl:template name="PrintElementTypePrefix">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Technology_Node'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Technology Node')"/>
				</span>
				<br/>
			</xsl:when>
			<xsl:when test="$elementType = 'Hardware_Instance'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Hardware Instance')"/>
				</span>
				<br/>
			</xsl:when>
			<xsl:when test="$elementType = 'Infrastructure_Software_Instance'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Infrastructure Software')"/>
				</span>
				<br/>
			</xsl:when>
			<xsl:when test="$elementType = 'Application_Software_Instance'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Application Software')"/>
				</span>
				<br/>
			</xsl:when>
			<xsl:when test="$elementType = 'Information_Store_Instance'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Information Store')"/>
				</span>
				<br/>
			</xsl:when>
			<xsl:when test="$elementType = 'Technology_Deployment_Group'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Technology Deployment')"/>
				</span>
				<br/>
			</xsl:when>
		</xsl:choose>
	...
	<xsl:template name="PrintElementDisplayLabel">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Technology_Node'">
				<xsl:variable name="aName" select="translate($theElement/own_slot_value[slot_reference = 'name']/value, '.', ' ')"/>
				<xsl:call-template name="RenderInstanceLink">
					<xsl:with-param name="theSubjectInstance" select="$theElement"/>
					<xsl:with-param name="theXML" select="$reposXML"/>
					<xsl:with-param name="viewScopeTerms" select="$viewScopeTerms"/>
					<xsl:with-param name="displayString" select="(replace($aName, '::', ' '))"/>
					<xsl:with-param name="anchorClass" select="'text-white'"/>
				</xsl:call-template>
				<xsl:text> in </xsl:text>
				<xsl:variable name="aSite" select="/node()/simple_instance[name = $theElement/own_slot_value[slot_reference = 'technology_deployment_located_at']/value]"/>
				<xsl:value-of select="$aSite/own_slot_value[slot_reference = 'name']/value"/>
			</xsl:when>
			<xsl:when test="$elementType = 'Technology_Deployment_Group'">
				<xsl:call-template name="RenderInstanceLink">
					<xsl:with-param name="theSubjectInstance" select="$theElement"/>
	...
	<xsl:template name="BuildPage">
		...
		<xsl:variable name="allGapElements" select="/node()/simple_instance[(type = 'Technology_Node') or (type = 'Technology_Deployment_Group') or (type = 'Hardware_Instance') or (type = 'Infrastructure_Software_Instance') or (type = 'Application_Software_Instance') or (type = 'Information_Store_Instance')]"/>
		...
		<xsl:variable name="currentStateElements" select="$allGapElements[name = $startArchState/own_slot_value[slot_reference = 'arch_state_technology_physical']/value]"/>
		<xsl:variable name="targetStateElements" select="$allGapElements[name = $targetArchState/own_slot_value[slot_reference = 'arch_state_technology_physical']/value]"/>
