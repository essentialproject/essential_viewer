...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:template match="knowledge_base">
		...
		<xsl:choose>
			<xsl:when test="string-length($param4) > 0">
				<xsl:variable name="roadmapConstant" select="/node()/simple_instance[own_slot_value[slot_reference = 'report_constant_short_name']/value = 'Technology_Roadmap']"/>
				...
				<xsl:variable name="pageLabel" select="concat('Technology Roadmap (', $orgScopeTermName, ')')"/>
			...
			<xsl:otherwise>
				<xsl:variable name="roadmapConstant" select="/node()/simple_instance[(own_slot_value[slot_reference = 'report_constant_short_name']/value = 'Technology_Roadmap')][1]"/>
				<xsl:variable name="roadmap" select="/node()/simple_instance[name = $roadmapConstant/own_slot_value[slot_reference = 'report_constant_ea_elements']/value]"/>
				<xsl:call-template name="BuildPage">
					<xsl:with-param name="roadmap" select="$roadmap"/>
				</xsl:call-template>
	...
	<xsl:template name="BuildPage">
		<xsl:param name="pageLabel">
			<xsl:value-of select="eas:i18n('Technology Roadmap')"/>
		</xsl:param>
			...
			<head>
				...
				<script type="text/javascript">
						...
						<xsl:for-each select="$roadmapMilestones">
							...
							<xsl:variable name="linkURL">
								<xsl:call-template name="RenderFullLinkText">
									...
									<xsl:with-param name="theXSL" select="'technology/core_tl_tech_milestone_summary.xsl'"/>
							...
							'description': '<strong><xsl:value-of select="eas:i18n('Milestone Summary')"/></strong><br/><xsl:value-of select="$currentStateDescription"/><br/><br/><strong>Target Date: </strong><xsl:value-of select="$displayStartDate"/><xsl:text>',
					...
					<xsl:for-each select="$strategicPlans">
						...
						'description': '<strong><xsl:value-of select="eas:i18n('Strategic Plan Summary')"/></strong><br/><xsl:value-of select="$currentPlanDescription"/><br/><br/><strong>Start Date: </strong><xsl:value-of select="$displayStartDate"/><br/><strong>End Date: </strong><xsl:value-of select="$displayEndDate"/><xsl:text>',
							'link': '</xsl:text><xsl:value-of select="$linkURL"/><xsl:text>',
