...
<xsl:stylesheet xpath-default-namespace="http://protege.stanford.edu/xml" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:functx="http://www.functx.com" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<!-- 20.01.2012	JWC	Updated for Technology Interoperability Change Analysis -->
	...
	<!-- param1 = the instance ID of the technology component -->
	...
	<xsl:variable name="roadmapConstantName" select="'Technology_Roadmap'"/>

	<!-- CHANGE THIS Change Analysis LABEL TO THE NAME OF THE Change Analysis -->
	<xsl:variable name="gapReportLabel">
		<xsl:value-of select="eas:i18n('Technology Change Analysis - Technology Interoperability')"/>
	</xsl:variable>

	<!-- CHANGE THIS TEXT TO LIST THE TYPES OF ELEMENTS IN SCOPE FOR THE Change Analysis (USED IN DESCRIPTIONS) -->
	<xsl:variable name="gapReportElementDesc" select="'Technology Component Dependencies'"/>
	...
	<xsl:template name="PrintElementTypePrefix">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Technology_Component'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Technology Component')"/>
				</span>
				<br/>
			</xsl:when>
			<xsl:when test="$elementType = ':TCU-TO-TCU-RELATION'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Technology Dependency')"/>
				</span>
				<br/>
			</xsl:when>
		</xsl:choose>
	...
	<xsl:template name="BuildPage">
		...
		<!-- Get all the Usages of the selected Component -->
		<xsl:variable name="techCompUsages" select="/node()/simple_instance[own_slot_value[slot_reference = 'usage_of_technology_component']/value = $param1]"/>
		<!-- All gap elements are all the :TCU-TO-TCU-RELATIONs in which the usages appear in the :TO or :FROM slots -->
		<xsl:variable name="allGapElements" select="/node()/simple_instance[(type = ':TCU-TO-TCU-RELATION') and ((own_slot_value[slot_reference = ':TO']/value = $techCompUsages/name) or (own_slot_value[slot_reference = ':FROM']/value = $techCompUsages/name))]"/>
		<xsl:variable name="strategicPlans" select="/node()/simple_instance[(type = 'Technology_Strategic_Plan') and (name = $roadmapRelations/own_slot_value[slot_reference = ':roadmap_strategic_plans']/value)]"/>
		<xsl:variable name="currentStateElements" select="$allGapElements[name = $startArchState/own_slot_value[slot_reference = 'arch_state_technology_logical']/value]"/>
		<xsl:variable name="targetStateElements" select="$allGapElements[name = $targetArchState/own_slot_value[slot_reference = 'arch_state_technology_logical']/value]"/>
