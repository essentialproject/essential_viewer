...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="linkClasses" select="('Technology_Domain','Technology_Capability','Technology_Component', 'Technology_Product')"/>
	...
	<xsl:template match="knowledge_base">
		...
		<html lang="en">
          ...
          <xsl:with-param name="requiresDataTables" select="false()"/>
				...
				<meta charset="UTF-8" />
				<title><xsl:value-of select="eas:i18n('Technology Domain Catalogue Table')"/></title>
			...
			<body role="document" aria-labelledby="main-heading">
				...
				<div class="container-fluid">
					<div class="row">
						<div class="col-xs-12">
							<div class="page-header">
								<h1 id="main-heading">
									...
									<span class="text-darkgrey"><xsl:value-of select="eas:i18n('Technology Domain Catalogue Table')"/></span>
								</h1>
							</div>
						</div>
                        ...
                        <div class="col-xs-12"> 
                           <table class="table table-striped table-condensed top-10 small dataTable" role="grid" aria-describedby="ebfw-editor-selection-table_info" id="dt_TechDomTable"> 
								<thead>
									<tr id="headerRow">
...
var meta = [
        ...
        "menuId": "techDomainGenMenu"
		            ...
		            var catalogueTable;
                        ...
                        $(document).ready(function() {
							...
							<!-- DEFINE APIS -->
                            apiList=['techProdSvc','ImpTechCompApi','ImpTechCapApi','ImpTechDomApi'];

                            async function executeFetchAndRender() {
                                try {
                            let responses = await fetchAndRenderData(apiList);
							
                            ({  techProdSvc,ImpTechCompApi,ImpTechCapApi,ImpTechDomApi } = responses);
							...
							//console.log('responses', responses) 
							 ...
							 TechDomainData = responses.ImpTechDomApi.technology_domains;
 ...
 colSettings=[
			...
			},
			{
				...
				$('#dt_TechDomTable tfoot th').each(function () {
		            var title = $(this).text();
...
// Initialize DataTable with dynamic columns
				table = $("#dt_TechDomTable").DataTable({
				"paging": false,
				"deferRender": true,
				"scrollY": 350,
        		"scrollX": true,
			...
			//	"data": workingArr,
				...
				stateSaveCallback: function(settings, data) {
					...
					localStorage.setItem('DataTables_Tech_Dom_Tab' + settings.sInstance, JSON.stringify(data))
				},
				stateLoadCallback: function(settings) {

					var data = JSON.parse(localStorage.getItem('DataTables_Tech_Dom_Tab' + settings.sInstance));
...
<!-- start tech dom to Tech capability mapping -->

    scopedData.techdoms.forEach(techDom => {
        // Initialize a new property to store the matched tech capabilities.
        // This ensures every capability object has the 'tech_capabilities' property.
        techDom.tech_capabilities = [];

        // Iterate over each technology capability to find matches.
        scopedData.techcaps.forEach(techCap => {

				
       
                // Check if the capability includes the current domain's name.
                if (techCap.domainId.includes(techDom.id)) {
				
                    // If a match is found, push the entire capability object into the
                    // 'tech_capabilties' array of the current capability.
                    techDom.tech_capabilities.push(techCap);
                }
            
        });
    });

<!-- end tech dom to Tech capability mapping -->
...
for (var i = 0; scopedData.techdoms.length > i; i += 1) {

			
        
            <!-- start irs is required to declare core item class for smart link context menus -->
		            irs = scopedData.techdoms[i];
				    irs['className']='Technology_Domain';
           <!-- end irs is required to declare core item class for smart link context menus -->
			
			
		            //Apply handlebars template
                
				selectHTML=selectTemplate(scopedData.techdoms[i]);
				selectNameHTML=selectNameTemplate(scopedData.techdoms[i]);
   
                techCapsHTML=techCapsTemplate(scopedData.techdoms[i]);
                techCompsHTML=techCompsTemplate(scopedData.techdoms[i]);

				techProdsHTML=techProdsTemplate(scopedData.techdoms[i]);

					tblData.push({"select":selectHTML,"name":selectNameHTML,"desc":scopedData.techdoms[i].description,"techcap":techCapsHTML,"techcomp":techCompsHTML,"productname":techProdsHTML});
           ...
           function setCatalogueTable(scopedData) {
             ...
             var redrawView = function () {
				...
				let scopedTechDomList = [];
				let scopedTechProdList = [];
				
				TechDomainData.forEach((d) => {
					scopedTechDomList.push(d)
				});
            ...
            //let scopedTechDoms = essScopeResources(scopedTechDomList, [techCapScopingDef].concat(dynamicAppFilterDefs), typeTechDom);
	        ...
	        let scopedTechProds = essScopeResources(scopedTechProdList, [techOrgScopingDef, visibilityDef].concat(dynamicAppFilterDefs), typeTechProd); 

				let viewArray = {}; 
				<!-- viewArray['type'] = "<xsl:value-of select="$repYN"/>"; -->
				viewArray['techdoms'] = scopedTechDomList;
				viewArray['techcaps'] = scopedTechCapList;
				viewArray['techcomps'] = scopedTechCompList;
