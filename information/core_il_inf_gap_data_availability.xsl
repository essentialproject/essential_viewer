...
<xsl:stylesheet xpath-default-namespace="http://protege.stanford.edu/xml" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:functx="http://www.functx.com" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<!-- param1 = the id of the Data Subject whose Data Representations are being described  -->
	<xsl:param name="param1"/>
	<xsl:variable name="dataObject" select="/node()/simple_instance[name = $param1]"/>
	<xsl:variable name="doName" select="$dataObject/own_slot_value[slot_reference = 'name']/value"/>
	...
	<xsl:variable name="linkClasses" select="('Data_Representation')"/>
	...
	<xsl:variable name="genericPageLabel">
		<xsl:value-of select="eas:i18n('Data Availability Change Analysis')"/>
	</xsl:variable>
	<xsl:variable name="gapReportLabel" select="concat($genericPageLabel, ' - ', $doName)"/>

	<!-- CHANGE THIS TEXT TO LIST THE TYPES OF ELEMENTS IN SCOPE FOR THE Change Analysis (USED IN DESCRIPTIONS) -->
	<xsl:variable name="gapReportElementDesc" select="'Data Representations'"/>
	...
	<xsl:template name="PrintElementTypePrefix">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Data_Representation'">
	...
	<xsl:template name="PrintElementDisplayLabel">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Data_Representation'">
				<!--<xsl:variable name="dataRepName" select="$theElement/own_slot_value[slot_reference='name']/value"></xsl:variable>-->
	...
	<xsl:template name="BuildPage">
		...
		<!-- All gap elements are all the Data Representations which are asssociated with the selected Data Object -->
		...
		<xsl:variable name="allGapElements" select="/node()/simple_instance[own_slot_value[slot_reference = 'implemented_data_object']/value = $allDataObjects/name]"/>
		<xsl:variable name="strategicPlans" select="/node()/simple_instance[(type = 'Information_Strategic_Plan') and (name = $roadmapRelations/own_slot_value[slot_reference = ':roadmap_strategic_plans']/value)]"/>
		<xsl:variable name="currentStateElements" select="$allGapElements[name = $startArchState/own_slot_value[slot_reference = 'arch_state_information_logical']/value]"/>
		<xsl:variable name="targetStateElements" select="$allGapElements[name = $targetArchState/own_slot_value[slot_reference = 'arch_state_information_logical']/value]"/>
