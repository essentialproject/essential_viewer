...
<xsl:stylesheet xpath-default-namespace="http://protege.stanford.edu/xml" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:functx="http://www.functx.com" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<!--<xsl:include href="../information/menus/core_data_object_menu.xsl"></xsl:include>-->

	<xsl:output method="html" omit-xml-declaration="yes" indent="yes"/>

	<!-- param1 = the id of the Data Subject whose Data Objects' Security Policies are being described  -->
	<xsl:param name="param1"/>
	<xsl:variable name="dataSubject" select="/node()/simple_instance[name = $param1]"/>
	<xsl:variable name="dsName" select="$dataSubject/own_slot_value[slot_reference = 'name']/value"/>
	...
	<xsl:variable name="linkClasses" select="('Info_Data_Management_Policy', 'Security_Policy', 'Data_Object')"/>
	...
	<xsl:variable name="roadmapConstantName" select="'Data_Roadmap'"/>

	<!-- CHANGE THIS Change Analysis LABEL TO THE NAME OF THE Change Analysis -->
	<xsl:variable name="genericPageLabel">
		<xsl:value-of select="eas:i18n('Data Security Change Analysis')"/>
	</xsl:variable>
	<xsl:variable name="gapReportLabel" select="concat($genericPageLabel, ' - ', $dsName)"/>

	<!-- CHANGE THIS TEXT TO LIST THE TYPES OF ELEMENTS IN SCOPE FOR THE Change Analysis (USED IN DESCRIPTIONS) -->
	<xsl:variable name="gapReportElementDesc" select="'Data Subjects accessed by Actor Roles'"/>
	...
	<xsl:variable name="allRoles" select="/node()/simple_instance[(type = 'Individual_Business_Role') or (type = 'Group_Business_Role')]"/>
	...
	<xsl:template name="PrintElementTypePrefix">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Security_Policy'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Access Right')"/>
				</span>
				<br/>
			</xsl:when>
			<xsl:when test="$elementType = 'Info_Data_Management_Policy'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Responsibility')"/>
				</span>
				<br/>
			</xsl:when>
		</xsl:choose>
	...
	<xsl:template name="PrintElementDisplayLabel">
		...
		<xsl:param name="elementType"/>
		<xsl:choose>
			<xsl:when test="$theElement/type = 'Security_Policy'">
				<xsl:variable name="spAction" select="$allSecuredActions[name = $theElement/own_slot_value[slot_reference = 'sp_action']/value]"/>
				<xsl:variable name="spDataObjects" select="$allDataObjects[name = $theElement/own_slot_value[slot_reference = 'sp_resources']/value]"/>
				<xsl:variable name="spName" select="$theElement/own_slot_value[slot_reference = 'name']/value"/>
				<xsl:choose>
					<xsl:when test="count($spDataObjects) > 0">
						<xsl:variable name="spActionName" select="$spAction/own_slot_value[slot_reference = 'enumeration_value']/value"/>
						<xsl:variable name="objectListLabel">
							<xsl:for-each select="$spDataObjects">
								<xsl:value-of select="current()/own_slot_value[slot_reference = 'name']/value"/>
								<xsl:if test="position() != count($spDataObjects)"> - </xsl:if>
							</xsl:for-each>
						</xsl:variable>
						...
						</xsl:call-template>&#160;(<xsl:value-of select="$spActionName"/>) <xsl:text disable-output-escaping="yes">&lt;/br></xsl:text>
						<xsl:for-each select="$spDataObjects">
							<xsl:call-template name="RenderInstanceLink">
			...
			<xsl:when test="$theElement/type = 'Info_Data_Management_Policy'">
				<xsl:variable name="role" select="$allRoles[name = $theElement/own_slot_value[slot_reference = 'sp_action']/value]"/>
				<xsl:variable name="responsibiility" select="$allRoles[name = $theElement/own_slot_value[slot_reference = 'dmp_responsibility']/value]"/>
				<xsl:variable name="dmpDataObject" select="$allDataObjects[name = $theElement/own_slot_value[slot_reference = 'dmp_assigned_info_data_object']/value]"/>
				<xsl:variable name="respName" select="$responsibiility/own_slot_value[slot_reference = 'name']/value"/>
				<xsl:call-template name="RenderInstanceLink">
					<xsl:with-param name="theSubjectInstance" select="$dmpDataObject"/>
					<xsl:with-param name="theXML" select="$reposXML"/>
					<xsl:with-param name="viewScopeTerms" select="$viewScopeTerms"/>
					<xsl:with-param name="anchorClass" select="'text-white'"/>
				</xsl:call-template>
				<span>&#160;(<xsl:value-of select="$respName"/>)</span>
			</xsl:when>
		</xsl:choose>
	</xsl:template>

	<!-- This template is used to allow alternative slots or even the names of related instances to be used as the label for elements in scope -->
	<xsl:template name="PrintElementDisplayLabelOLD">
		<xsl:param name="theElement"/>
		<xsl:param name="elementType"/>
		<xsl:choose>
			<xsl:when test="$theElement/type = 'Security_Policy'">
				...
				<xsl:choose>
					<xsl:when test="count($spDataObjects) > 0">
						<xsl:variable name="spActionName" select="$spAction/own_slot_value[slot_reference = 'enumeration_value']/value"/>
						<!--<xsl:variable name="elementLabel"><xsl:value-of select="$spDataObjects[1]/own_slot_value[slot_reference='name']/value"/>&#160;(<xsl:value-of select="$spActionName" />)</xsl:variable>-->
						<xsl:call-template name="RenderInstanceLink">
							<xsl:with-param name="theSubjectInstance" select="$spDataObjects[1]"/>
							<xsl:with-param name="theXML" select="$reposXML"/>
							<xsl:with-param name="viewScopeTerms" select="$viewScopeTerms"/>
							<xsl:with-param name="anchorClass" select="'text-white'"/>
							<!--<xsl:with-param name="displayString" select="$elementLabel"/>-->
						</xsl:call-template>
						<!--<span>&#160;(<xsl:value-of select="$spActionName" />)</span>-->
					</xsl:when>
					<xsl:otherwise>
						<xsl:call-template name="RenderInstanceLink">
							<xsl:with-param name="theSubjectInstance" select="$theElement"/>
	...
	<xsl:template name="BuildPage">
		...
		<!-- All gap elements are all the Data Management Policies which are asssociated with the selected Data Object -->
		<xsl:variable name="allGapElements" select="/node()/simple_instance[(own_slot_value[slot_reference = 'sp_resources']/value = $allDataObjects/name) or (own_slot_value[slot_reference = 'dmp_assigned_data_object']/value = $allDataObjects/name)]"/>
		<xsl:variable name="strategicPlans" select="/node()/simple_instance[(type = 'Security_Strategic_Plan') and (name = $roadmapRelations/own_slot_value[slot_reference = ':roadmap_strategic_plans']/value)]"/>
		<xsl:variable name="targetStateElements" select="$allGapElements[name = $targetArchState/own_slot_value[slot_reference = 'arch_state_security_management']/value]"/>
		<xsl:variable name="currentStateElements" select="$allGapElements[name = $startArchState/own_slot_value[slot_reference = 'arch_state_security_management']/value]"/>
								...
								<div class="content-section">
									<p>
										<xsl:value-of select="eas:i18n('The table below provides a comparative analysis between')"/>&#160;<strong>
											<xsl:value-of select="eas:i18n('Target and Baseline')"/>&#160;<xsl:value-of select="$gapReportElementDesc"/>
										</strong>&#160;<xsl:value-of select="eas:i18n('by indicating what needs to change across')"/>&#160;<xsl:value-of select="$orgName"/>
									</p>
