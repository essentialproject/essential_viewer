...
<xsl:stylesheet xpath-default-namespace="http://protege.stanford.edu/xml" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:functx="http://www.functx.com" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="linkClasses" select="('Data_Object', 'Data_Subject')"/>
	...
	<xsl:variable name="gapReportLabel"><xsl:value-of select="eas:i18n('Data Scarcity / Redundancy Change Analysis for')"/>&#160; <xsl:value-of select="$dsName"/></xsl:variable>

	<!-- CHANGE THIS TEXT TO LIST THE TYPES OF ELEMENTS IN SCOPE FOR THE Change Analysis (USED IN DESCRIPTIONS) -->
	<xsl:variable name="gapReportElementDesc" select="'Logical Data Objects'"/>
	...
	<xsl:template name="PrintElementTypePrefix">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Data_Object'">
	...
	<xsl:template name="PrintElementDisplayLabel">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Data_Object'">
				...
				<!--<xsl:variable name="dataObjName" select="$theElement/own_slot_value[slot_reference='name']/value" />
				<a id="{$dataObjName}" class="context-menu-dataObject menu-1 text-white">
	...
	<xsl:template name="BuildPage">
		...
		<xsl:variable name="allGapElements" select="/node()/simple_instance[own_slot_value[slot_reference = 'defined_by_data_subject']/value = $param1]"/>
						...
						<div class="col-xs-12">
							...
							<div class="content-section">
								<p>
									...
									<xsl:value-of select="eas:i18n('by indicating what needs to change across')"/>&#160; <xsl:value-of select="$orgName"/>. </p>
