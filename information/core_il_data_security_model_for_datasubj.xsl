...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<!-- param1 = the data subject whose related Data Objects are being mapped to business roles -->
	...
	<xsl:template name="BuildPage">
		<xsl:param name="pageLabel"><xsl:value-of select="eas:i18n('Data Security Model for')"/>&#160;<xsl:value-of select="$currentDataSubjectName"/></xsl:param>
				...
				<div class="container-fluid">
					<div class="row">
						...
						<div class="col-xs-12">
							...
							<p><xsl:value-of select="eas:i18n('The following matrix maps logical Data Objects for')"/>&#160;<strong><xsl:value-of select="$currentDataSubjectName"/></strong>&#160;<xsl:value-of select="eas:i18n('to the Business Roles in use across')"/><xsl:value-of select="$orgName"/>
							</p>
							<p>
	...
	<xsl:template match="node()" mode="DataObject">
		...
		<th class="cellWidth-140 vAlignMiddle">
			<!--<a id="{$doName}" class="text-white context-menu-dataObject menu-1">
	...
	<xsl:template name="BusinessRole2DataCell">
		...
		<xsl:variable name="relevantSecurityPolicies" select="$allSecurityPolicies[(own_slot_value[slot_reference = 'sp_actor']/value = $businessRole/name) and (own_slot_value[slot_reference = 'sp_resources']/value = $dataObject/name)]"/>
