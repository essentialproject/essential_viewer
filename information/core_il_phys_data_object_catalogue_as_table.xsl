...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="linkClasses" select="('Physical_Data_Object', 'Data_Object')"/>
	<!-- END GENERIC CATALOGUE SETUP VARIABES -->

	<xsl:variable name="physDataObjects" select="/node()/simple_instance[type = 'Physical_Data_Object']"/>

	<xsl:variable name="pageLabel">
		<xsl:value-of select="eas:i18n('Physical Data Object Catalogue')"/>
	</xsl:variable>
	<xsl:variable name="physDataObjCount" select="count($physDataObjects)"/>
				...
				<div class="container-fluid">
					<div class="row">
						...
						<div class="col-xs-12">
							...
							<p>
								<xsl:value-of select="eas:i18n('Select a Physical Data Object to navigate to the required view')"/>
							</p>
		...
		<table class="table table-striped table-bordered" id="dt_physDataObj">
			<thead>
				<tr>
					...
					<th>
						<xsl:value-of select="eas:i18n('Contained In Information Store')"/>
					</th>
				</tr>
			</thead>
			<tfoot>
			...
			<tbody>
				<xsl:choose>
					<xsl:when test="string-length($viewScopeTermIds) > 0">
						<xsl:apply-templates mode="tableRow" select="$physDataObjects[own_slot_value[slot_reference = 'element_classified_by']/value]">
							<xsl:sort select="own_slot_value[slot_reference = 'name']/value"/>
						</xsl:apply-templates>
					</xsl:when>
					<xsl:otherwise>
						<xsl:apply-templates mode="tableRow" select="$physDataObjects">
							<xsl:sort select="own_slot_value[slot_reference = 'name']/value"/>
						</xsl:apply-templates>
					</xsl:otherwise>
				</xsl:choose>
	...
	<xsl:template match="node()" mode="tableRow">
		...
		<xsl:variable name="containedInInfoStore" select="$allInfoStores[name = current()/own_slot_value[slot_reference = 'contained_in_information_store']/value]"/>
		<tr>
			<td>
			...
			<td>
				...
				<ul>
					<xsl:for-each select="$containedInInfoStore">
						<li>
							<xsl:call-template name="RenderInstanceLink">
								<xsl:with-param name="theXML" select="$reposXML"/>
								<xsl:with-param name="theSubjectInstance" select="current()"/>
