...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:functx="http://www.functx.com">
	...
	<xsl:variable name="pageTitle" select="concat('Logical Model for Data Subject - ', $modelSubjectName)"/>
	...
	<xsl:variable name="dataObjectsForSubject" select="/node()/simple_instance[name = $modelSubject/own_slot_value[slot_reference = 'realised_by_data_objects']/value]"/>
	<xsl:variable name="inScopeDataAttributes" select="/node()/simple_instance[name = $dataObjectsForSubject/own_slot_value[slot_reference = 'contained_data_attributes']/value]"/>

	<xsl:variable name="otherDataObjects" select="$allDataObjects[not(name = $dataObjectsForSubject/name) and (name = $inScopeDataAttributes/own_slot_value[slot_reference = 'type_for_data_attribute']/value)]"/>
	...
	<xsl:variable name="childDataObjects" select="$dataObjectsForSubject[name = $dataObjectsForSubject/own_slot_value[slot_reference = 'data_object_specialisations']/value]"/>
					...
					<div class="row">
						<div class="col-xs-12">
							<h1 id="viewName">
								...
								<span class="text-darkgrey">Logical Model for Data Subject for </span>
	...
	<xsl:template name="modelScript">
		...
		<script>
			...
			var classes = {
				...
				<xsl:apply-templates mode="RenderDataObjectUML" select="$dataObjectsForSubject union $otherDataObjects"/>
			...
			var relations = [
				<xsl:apply-templates mode="RenderObjectAssociations" select="$objectDataAttributes"/><xsl:if test="count($childDataObjects) > 0">,</xsl:if>
	...
	<xsl:template mode="RenderDataObjectUML" match="node()">
		...
		<!-- JMK: call to translate could be avoided ? -->
	...
	<xsl:template mode="RenderObjectGeneralisations" match="node()">
		...
		<xsl:variable name="parentDataObject" select="$allDataObjects[name = current()/own_slot_value[slot_reference = 'data_object_generalisations']/value]"/>
		...
		<xsl:variable name="jsParentDataObjectName" select="eas:get_js_name_for_data_object($parentDataObject)"/> new uml.Generalization({ source: { id: classes.<xsl:value-of select="$jsChildDataObjectName"/>.id }, target: { id: classes['<xsl:value-of select="$jsParentDataObjectName"/>'].id }})<xsl:if test="not(position() = last())">,</xsl:if><xsl:text>
	...
	<xsl:function as="xs:string" name="eas:get_js_name_for_data_object">
		<xsl:param name="dataObject" as="element()"/>
