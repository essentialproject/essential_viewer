...
<xsl:stylesheet version="2.0" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview" xmlns:functx="http://www.functx.com" xmlns:pro="http://protege.stanford.edu/xml" xmlns:xalan="http://xml.apache.org/xslt" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xpath-default-namespace="http://protege.stanford.edu/xml">
	...
	<xsl:variable name="allStratPlans" select="/node()/simple_instance[type = 'Enterprise_Strategic_Plan']"/>
	...
	<xsl:variable name="thisReport" select="eas:get_report_by_name('Schroders: Business Strategy Footprint - 3 Level')"/>
	<xsl:variable name="basicQueryString">
		<xsl:call-template name="RenderLinkText">
			<!-- Hard code for now to work around repository data typo -->
			<!--<xsl:with-param name="theXSL" select="$thisReport/own_slot_value[slot_reference='report_xsl_filename']/value"/>-->
...
-->
	...
	<xsl:variable name="queryStringPrefix">
		<xsl:call-template name="RenderLinkText">
	...
	<!--Calculate the number of capabilities for max width-->
	<xsl:variable name="countFrontParentCaps">
		<xsl:call-template name="sizeCalc"/>
	</xsl:variable>
		...
		<html>
			<head>
				...
				<script type="text/javascript">
					...
					$(".threeColModel_HeaderObject").vAlign();
					$(".circleButton").vAlign();
					$(".buttonLabel").vAlign();
				...
				<style type="text/css">
					...
					.threeColModel_wideRowCaps{
					    ...
					    float: left;
					    clear: both;
					    width: <xsl:value-of select="$countFrontParentCaps*284"/>px;
					}
					...
					.threeColModel_LogoSection{
					    ...
					    box-sizing: content-box;
					    width: <xsl:value-of select="($countFrontParentCaps*284)-20"/>px;
					}
						...
						<div id="sectionDescription">
							<div class="col-xs-12">
								...
								<div class="content-section">
									...
									<div class="verticalSpacer_10px"/>
									<div class="simple-scroller"><xsl:call-template name="anchorModel"/></div>
								</div>
	...
	<xsl:template name="anchorModel">
		...
		<div class="threeColModel_wideColumnContainerCaps pull-left">
			...
			<div class="clear"/>
			<xsl:apply-templates mode="PrintTopLevelManageCap2" select="$topLevelManageBusCapRelss"/>
			<div class="threeColModel_wideRowCapsDivider"/>
			<xsl:apply-templates mode="PrintTopLevelFrontCap" select="$topLevelFrontBusCapRelss"/>
			<xsl:apply-templates mode="PrintTopLevelManageCap2" select="$topLevelBackBusCapRelss"/>
	...
	<xsl:template match="node()" mode="PrintTopLevelManageCap2">
		...
		<div class="threeColModel_wideRowCaps">
			...
			<xsl:apply-templates mode="PrintChildManageCap2" select="$childCaps">
				<xsl:sort select="number(own_slot_value[slot_reference = 'business_capability_index']/value)"/>
			</xsl:apply-templates>
		</div>
	...
	<xsl:template match="node()" mode="PrintChildManageCap2">
		...
		<xsl:variable name="containerDivStyle"> threeColModel_HeaderObjectContainer pull-left </xsl:variable>
		<xsl:variable name="chevronDivStyle"> threeColModel_HeaderObject small bg-primary8 </xsl:variable>
	...
	<xsl:template name="dynamicKey">
		...
		<div class="dynamicKey">
			<div class="threeColModel_dynamicKeyTitle small" style="width:150px;">
				<strong>
	...
	<xsl:template name="busCapProjectList">
		...
		<xsl:if test="count($inScopeProjects) > 0">
			...
			<xsl:for-each select="$inScopeProjects">
				<xsl:variable name="currentProj" select="current()"/>
				<div class="pull-left clear" style="width:250px;">
	...
	<!-- Function to calculate the size of the layout based on number of business capabilities
		Return 0 if there are none -->
	<xsl:template name="sizeCalc">
		<xsl:choose>			
			<xsl:when test="count($topLevelFrontBusCapRelss) > 0">
				<xsl:for-each select="$topLevelFrontBusCapRelss">
					<xsl:variable name="thisFrontBusCap" select="$allBusinessCaps[name = current()/own_slot_value[slot_reference = 'buscap_to_parent_child_buscap']/value]"/>
					<xsl:variable name="childCaps" select="$allBusinessCaps[name = $thisFrontBusCap/own_slot_value[slot_reference = 'contained_business_capabilities']/value]"/>
					<xsl:value-of select="count($childCaps)"/>
				</xsl:for-each>
			</xsl:when>
