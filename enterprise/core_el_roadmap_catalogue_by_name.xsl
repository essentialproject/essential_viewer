...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="instanceClassName" select="'Roadmap_Model'"/>
	...
	<xsl:variable name="roadmapListByLayerCatalogue" select="/node()/simple_instance[(type = 'Report') and (own_slot_value[slot_reference = 'name']/value = 'Core: Roadmap Catalogue by Layer')]"/>
	...
	<xsl:template name="BuildPage">
		<xsl:param name="pageLabel">
			<xsl:value-of select="eas:i18n('Roadmap Catalogue by Name')"/>
		</xsl:param>
				...
				<div class="container-fluid">
								...
								<div class="altViewName">
									...
									<span class="text-darkgrey">
										<xsl:call-template name="RenderCatalogueLink">
											<xsl:with-param name="theCatalogue" select="$roadmapListByLayerCatalogue"/>
											<xsl:with-param name="theXML" select="$reposXML"/>
											<xsl:with-param name="viewScopeTerms" select="$viewScopeTerms"/>
											<xsl:with-param name="targetReport" select="$targetReport"/>
											<xsl:with-param name="targetMenu" select="$targetMenu"/>
											<xsl:with-param name="displayString" select="'Architecture Layer'"/>
										</xsl:call-template>
									</span>
								</div>
							</div>
					...
					<div class="row">
						<div class="col-xs-12">
							...
							<p><xsl:value-of select="eas:i18n('Please select a Roadmap Model to navigate to the required view')"/>.</p>
