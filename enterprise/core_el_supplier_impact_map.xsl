...
<!-- order here has to map the markers order -->     
    var focusSupplier=[];
    supplierJSON=[<xsl:apply-templates select="$supplier" mode="supplier"/>];
    capabilityJSON=[<xsl:apply-templates select="$rootLevelBusCaps" mode="busCaps"/>];
    plansJSON=[<xsl:apply-templates select="$allStratPlans" mode="stratPlans"/>]      
	...
	//console.log(thisApp)
       if( thisApp[0]){}else{ var thisTech=thisSupplier[0].technologies.filter(function(d){
                            d.impacted.filter(function(e){
                                 if(e.caps){   
                                 e.caps.forEach(function(c){
                             
                        capabilityJSON.forEach(function(c1){
                            if(c1.id===c.id){}
                                 c1.subCaps.forEach(function(c2){
                                    if(c2.id===c.id){$('.'+c2.id).css({'border-bottom':'3pt solid red','box-shadow': '5px 5px 5px  #ccc'});
                                        }
                                     c2.relatedCaps.forEach(function(c3){
                                        if(c3.id===c.id){$('.'+c2.id).css({'border-bottom':'3pt solid red','box-shadow': '5px 5px 5px #ccc'});
                                                    }
                                                })
                                            })
                                        })
      ...
      if( thisApp[0]){
                    thisApp[0].capabilitiesImpacted.forEach(function(e){ 
                        capabilityJSON.forEach(function(c1){
                            if(c1.id===e.id){}
                                 c1.subCaps.forEach(function(c2){
                                    if(c2.id===e.id){$('.'+c2.id).css({'border-bottom':'3pt solid red','box-shadow': '5px 5px 5px  #ccc'});
                                        }
                                     c2.relatedCaps.forEach(function(c3){
                                        if(c3.id===e.id){$('.'+c2.id).css({'border-bottom':'3pt solid red','box-shadow': '5px 5px 5px  #ccc'});
                                                    }
                                                })
                                            })
                                        })   
...
{{/each}}
	...
	<xsl:template name="supHandlebarsTemplate">
		<script id="supplier-template" type="text/x-handlebars-template">
            ...
            {{#each apps}}
                <div><xsl:attribute name="class">appList</xsl:attribute><xsl:attribute name="data-easid">{{this.id}}</xsl:attribute> <i class="fa fa-desktop"/>  <span style="font-size:1.1em"> {{{this.name}}}</span>
            ...
            {{#each technologies}}
            <div class="appList"><xsl:attribute name="data-easid">{{this.id}}</xsl:attribute> <i class="fa fa-server"/> <span style="font-size:1.1em"> {{{this.name}}}</span>
            {{#each impacted}}
             ...
             {{#each impacted}}    
                {{#each apps}}
                <div><xsl:attribute name="class">appListBg</xsl:attribute><i class="fa fa-desktop"/> {{{this.name}}}</div> 
                {{/each}}    
