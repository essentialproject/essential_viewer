...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="idea" select="/node()/simple_instance[type = 'Idea'][own_slot_value[slot_reference = 'idea_proposed_changes']/value = $ideaElement/name]"/>
	<xsl:variable name="businessNeed" select="/node()/simple_instance[type = 'Business_Need'][own_slot_value[slot_reference = 'bn_ideas']/value = $idea/name]"/>
	<xsl:variable name="costChange" select="/node()/simple_instance[type = 'Cost_Component_Change'][name = $businessNeed/own_slot_value[slot_reference = 'bn_target_cost_changes']/value]"/>
	...
	<xsl:variable name="revenueChange" select="/node()/simple_instance[type = 'Revenue_Component_Change'][name = $businessNeed/own_slot_value[slot_reference = 'bn_target_revenue_changes']/value]"/>
	...
	<xsl:variable name="serviceQualityChange" select="/node()/simple_instance[type = 'Service_Quality_Change'][name = $businessNeed/own_slot_value[slot_reference = 'bn_target_service_quality_changes']/value]"/>
	...
	<script>
       var externalKPIData;
          <!--   externalKPIData=[{'projectID':'eas_prj_HH_baseline_rep_v11gv1_Class40036','actualKpis':[{'kpiID':'kpi1','actualValue':10}]},{'projectID':'eas_prj_HH_baseline_rep_v11e_Class41374','actualKpis':[{'kpiID':'kpi1','actualValue':10}]}]; -->
            ...
            kpiJSON2=[{'id':'kpi1','name':'Sales By Region','icon':'usd'}, {'id':'kpi2','name':'Profit Margin','icon':'line-chart'}, {'id':'kpi3','name':'Customer Satisfaction &amp; Retention'}, {'id':'kpi4','name':'Annual Customer Growth'}];
						...
						<div class="col-xs-9">
							{{#each this.kpis}}
								<div class="row">
									<div class="col-xs-4">
										<span>
											<i><xsl:attribute name="class">fa fa-{{icon}} right-5</xsl:attribute></i>
											{{kpiName}}
										</span>
									</div>
									<div class="col-xs-2">
									...
									<div class="col-xs-2">
										<span>
											<i class="fa fa-circle-o right-5"><xsl:attribute name="style">{{status}}</xsl:attribute></i>
											{{actual}}
										</span> 
									</div>
									<div class="col-xs-2">
										<span>
											<i><xsl:attribute name="style">color:{{status}}</xsl:attribute><xsl:attribute name="class">fa fa-{{statusIcon}} right-5</xsl:attribute></i>
											{{diff}}%
										</span>
									</div>
								</div>
	...
	<xsl:template match="node()" mode="projectStructure">
		...
		<xsl:variable name="thisbusinessNeed" select="$businessNeed[own_slot_value[slot_reference = 'bn_ideas']/value = $thisidea/name]"/>
		<xsl:variable name="thiscostChange" select="$costChange[name = $thisbusinessNeed/own_slot_value[slot_reference = 'bn_target_cost_changes']/value]"/>
		...
		<xsl:variable name="thisrevenueChange" select="$revenueChange[name = $thisbusinessNeed/own_slot_value[slot_reference = 'bn_target_revenue_changes']/value]"/>
		...
		<xsl:variable name="thisserviceQualityChange" select="$serviceQualityChange[name = $thisbusinessNeed/own_slot_value[slot_reference = 'bn_target_service_quality_changes']/value]"/>
		<xsl:variable name="thisserviceQualities" select="$serviceQualities[name = $thisserviceQualityChange/own_slot_value[slot_reference = 'sqc_service_quality']/value]"/> {"name":"<xsl:value-of select="current()/own_slot_value[slot_reference = 'name']/value"/>","id":"<xsl:value-of select="eas:getSafeJSString(current()/name)"/>", "kpis":[<xsl:if test="$thiscostComponent"><xsl:apply-templates select="$thiscostComponent" mode="kpiList"><xsl:with-param name="valueFor" select="$thiscostChange"/></xsl:apply-templates></xsl:if>
		<xsl:if test="$thisserviceQualities"><xsl:apply-templates select="$thisserviceQualities" mode="kpiList"><xsl:with-param name="valueFor" select="$thisserviceQualityChange"/></xsl:apply-templates></xsl:if>
		<xsl:if test="$thisrevenueComponent"><xsl:apply-templates select="$thisrevenueComponent" mode="kpiList"><xsl:with-param name="valueFor" select="$thisrevenueChange"/></xsl:apply-templates></xsl:if>]},</xsl:template>
	<xsl:template match="node()" mode="getServiceQualities">
		<xsl:variable name="thisStyle" select="$styleForKPIs[name = current()/own_slot_value[slot_reference = 'element_styling_classes']/value]"/> {'id':'<xsl:value-of select="eas:getSafeJSString(current()/name)"/>','name':'<xsl:value-of select="current()/own_slot_value[slot_reference = 'name']/value"/>','icon':'<xsl:value-of select="current()/own_slot_value[slot_reference = 'element_style_icon']/value"/><xsl:value-of select="$thisStyle[0]/own_slot_value[slot_reference = 'element_style_icon']/value"/>'} </xsl:template>

	<xsl:template match="node()" mode="kpiList">
		<xsl:param name="valueFor"/> {"id":"<xsl:value-of select="current()/name"/>","name":"<xsl:value-of select="current()/own_slot_value[slot_reference = 'name']/value"/>","value":"<xsl:value-of select="$valueFor/own_slot_value[slot_reference = 'change_percentage']/value"/>"}, </xsl:template>
