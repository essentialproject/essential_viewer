...
<script>
	...
	<xsl:template match="node()" mode="productList">
		...
		{"id":"<xsl:value-of select="translate($this/name, '.', '')"/>","product": "<xsl:value-of select="$this/own_slot_value[slot_reference = 'name']/value"/>","oid":"<xsl:value-of select="$this/name"/>","debug":"<xsl:value-of select="$thisContractRels/name"/>","stakeholders":[<xsl:apply-templates select="$allActors[name = $this/own_slot_value[slot_reference = 'stakeholders']/value]" mode="users"/>], 
		<xsl:choose>
	...
	<!--<xsl:template match="node()" mode="productList">
        ...
        {"id":"<xsl:value-of select="translate($this/name, '.', '')"/>","product": "<xsl:value-of select="$this/own_slot_value[slot_reference = 'name']/value"/>","oid":"<xsl:value-of select="$this/name"/>","debug":"<xsl:value-of select="$thisContracts/name"/>","stakeholders":[<xsl:apply-templates select="$allActors[name = $this/own_slot_value[slot_reference = 'stakeholders']/value]" mode="users"/>], 
		<xsl:choose>
	...
	<xsl:template match="node()" mode="subproductList">
		<xsl:variable name="this" select="current()"/>
		<xsl:variable name="thisContractRels" select="$allContractToElementRels[own_slot_value[slot_reference = 'contract_component_to_element']/value=current()/name]"/>
		 {"id":"<xsl:value-of select="translate($this/name, '.', '')"/>","product": "<xsl:value-of select="$this/own_slot_value[slot_reference = 'name']/value"/>","oid":"<xsl:value-of select="$this/name"/>","debug":"","stakeholders":[<xsl:apply-templates select="$allActors[name = $this/own_slot_value[slot_reference = 'stakeholders']/value]" mode="users"/>],
		<xsl:choose>
			<xsl:when test="$thisContractRels">
				<!--<xsl:variable name="thisLicenses" select="$licenses[name = $thisContracts/own_slot_value[slot_reference = 'compliance_obligation_licenses']/value]"/>-->
				<xsl:variable name="thisActualContracts" select="$allContracts[name = $thisContractRels/own_slot_value[slot_reference = 'contract_component_from_contract']/value]"/>
				...
				<xsl:variable name="Day" select="day-from-date(xs:date($endYear))"/>"Contract":"<xsl:value-of select="$currentContract/own_slot_value[slot_reference = 'name']/value"/>","Licence":"<xsl:value-of select="$currentLicense/own_slot_value[slot_reference = 'name']/value"/>","LicenceType":"<xsl:value-of select="$licenseType[name = $currentLicense/own_slot_value[slot_reference = 'license_type']/value]/own_slot_value[slot_reference = 'name']/value"/>","licenseOnContract":"<xsl:value-of select="$currentContractRel/own_slot_value[slot_reference = 'ccr_contracted_units']/value"/>","YearsOnContract":"<xsl:choose><xsl:when test="$contractRelLengthMonths > 0"><xsl:value-of select="$contractRelLengthMonths div 12"/></xsl:when><xsl:otherwise>0</xsl:otherwise></xsl:choose>","licenseCostContract":"<xsl:value-of select="format-number($currentContractRel/own_slot_value[slot_reference = 'ccr_total_annual_cost']/value, '##,###,###')"/>","licenseUnitPriceContract":"<xsl:value-of select="format-number($currentCostPerUnit, '##,###,###')"/>","month":"<xsl:value-of select="$Month"/>","year":"<xsl:value-of select="$Year"/>","EndDate":"<xsl:value-of select="$Day"/>/<xsl:value-of select="$Month"/>/<xsl:value-of select="$Year"/>","remaining":<xsl:value-of select="$remaining"/>,"rembgColor": "<xsl:choose><xsl:when test="$remaining &lt; 0">red</xsl:when>
					<xsl:when test="$remaining > 0 and $remaining &lt; 180">#f4c96a</xsl:when>
