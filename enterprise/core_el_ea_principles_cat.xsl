...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:template name="BuildPage">
		...
		<body>
			...
			<div class="container-fluid">
				<div class="row">
					...
					<xsl:variable name="panelID">#collapse
      ...
      -->
									...
									<div class="panel-body">
										<div class="container">
											...
											<div class="content-section">
												...
												<div class="col-xs-9">
													<p>
														<xsl:value-of select="eas:i18n('The following Business Principles guide and govern changes to the business architecture')" />.
													</p>
													<xsl:choose>
														<xsl:when test="string-length($viewScopeTermIds) > 0">
								...
								<div id="app" class="tab-pane fade">
									<div class="panel-body">
										<div class="container">
											...
											<div class="content-section">
												<p>
													<xsl:value-of select="eas:i18n('The following Application Principles guide and govern changes to the application architecture')" />.
												</p>
												<xsl:choose>
													<xsl:when test="string-length($viewScopeTermIds) > 0">
								...
								<div id="infodiv" class="tab-pane fade">
									<div class="panel-body">
										<div class="container">
											...
											<div class="content-section">
												<p>
													<xsl:value-of select="eas:i18n('The following Information Principles guide and govern changes to the information and data architecture')" />.
												</p>
								...
								<div id="techdiv" class="tab-pane fade">
									<div class="panel-body">
										<div class="container">
											...
											<div class="content-section">
												<p>
													<xsl:value-of select="eas:i18n('The following Technology Principles guide and govern changes to the technology architecture')" />.
												</p>
												<xsl:choose>
													<xsl:when test="string-length($viewScopeTermIds) > 0">
			...
			<div class="modal fade" id="principleAssessments" role="dialog">
				<div class="modal-dialog modal-lg">
					<div class="modal-content">
						<div class="modal-header">
							<h3 class="modal-title">Assessments</h3>
						</div>
			...
			<script>
				var principles = [ <xsl:apply-templates select = "$allPrinciples"
					mode = "principleRatings" />
				];
				...
				function butFunction(its) {
					...
					$(document).ready(function() {
						var principlesFragment = $("#principles-template").html();
						var principlesTemplate = Handlebars.compile(principlesFragment);
				...
				$('.tog').click(function() {
					$("i", this).toggleClass("fa-toggle-on fa-toggle-off");
						...
						$('.score').click(function() {
							...
							var toShow = principles.filter(function(d) {
								return d.principleID == thisid;
							})

							$('#principleAssessments').modal('show')
							$('#prin').html(principlesTemplate(toShow[0]));
			...
			<script id="principles-template" type="text/x-handlebars-template">
				<table class="table striped" id="assessTable">
	 ...
	 <tbody>
			{{#each this.scores}}
					...
					<tr>
					<td>{{this.assessment}}</td>
					<td>{{this.assessmentScore}}</td>
	...
	<xsl:template name="PrincipleImplications">
		...
		<table class="noBorders principleTable">
			<tbody>
				<tr>
					<th class="cellWidth-15pc vAlignTop">
				...
				<xsl:choose>
					<xsl:when test="count($aPrinciple/own_slot_value[slot_reference = $BusImplicationSlot]/value) > 0">
						<tr>
							<th class="vAlignTop">
								<xsl:value-of select="eas:i18n('Implications for the Business')" />:
							</th>
							<td>
								<ul>
				...
				<xsl:choose>
					<xsl:when test="count($aPrinciple/own_slot_value[slot_reference = $AppImplicationSlot]/value) > 0">
						<tr>
							<th class="vAlignTop">
								<xsl:value-of select="eas:i18n('Implications for Applications')" />:
							</th>
							<td>
								<ul>
									<xsl:call-template name="RenderMultiLangImplications">
				...
				<xsl:choose>
					<xsl:when test="count($aPrinciple/own_slot_value[slot_reference = $InfoImplicationSlot]/value) > 0">
						<tr>
							<th class="vAlignTop">
								<xsl:value-of select="eas:i18n('Implications for Information')" />:
							</th>
							<td>
								<ul>
									<xsl:call-template name="RenderMultiLangImplications">
				...
				<xsl:choose>
					<xsl:when test="count($aPrinciple/own_slot_value[slot_reference = $TechImplicationSlot]/value) > 0">
						<tr>
							<th class="vAlignTop">
								<xsl:value-of select="eas:i18n('Implications for Technology')" />:
							</th>
							<td>
								<ul>
									<xsl:call-template name="RenderMultiLangImplications">
	...
	<xsl:template name="dataScopePopoverContent">
		...
		<div class="text-default small hiddenDiv">
			...
			<ul>
				<xsl:apply-templates select="$currentObject" mode="list" />
			</ul>
	...
	<xsl:template name="assessementTable">
		...
		<table width="400px" style="table-layout: fixed;">
			<tr>
				<th width="200px"> </th>
				<th colspan="5" width="120px" style="font-size:8pt">Principle Adherence</th>
				<th></th>
			</tr>
			<tr>
				<th width="200px">Total Assessments</th>
				<th colspan="2" width="120px" style="font-size:8pt">Weak</th>
				<th colspan="2" width="80px" style="text-align:right;font-size:8pt">Strong</th>
				<th></th>
			</tr>
			<tr>
				...
				<td style="background-color:#fc6868;text-align:center;cursor:pointer" class="score">
					<xsl:attribute name="easid"><xsl:value-of select="eas:getSafeJSString($thisPrinciple/name)" />p1</xsl:attribute>
					<xsl:attribute name="pid"><xsl:value-of select="eas:getSafeJSString($thisPrinciple/name)" /></xsl:attribute>
				</td>
				<td style="background-color:#ffd23b;text-align:center;cursor:pointer" class="score">
					<xsl:attribute name="easid"><xsl:value-of select="eas:getSafeJSString($thisPrinciple/name)" />p2</xsl:attribute>
					<xsl:attribute name="pid"><xsl:value-of select="eas:getSafeJSString($thisPrinciple/name)" /></xsl:attribute>
				</td>
				<td style="background-color:#d8ff00;text-align:center;cursor:pointer" class="score">
					<xsl:attribute name="easid">
						<xsl:value-of select="eas:getSafeJSString($thisPrinciple/name)" />p3</xsl:attribute>
					<xsl:attribute name="pid"><xsl:value-of select="eas:getSafeJSString($thisPrinciple/name)" /></xsl:attribute>
				</td>

				<td style="background-color:#99fc51;text-align:center;cursor:pointer" class="score">
					<xsl:attribute name="easid"><xsl:value-of select="eas:getSafeJSString($thisPrinciple/name)" />p4</xsl:attribute>
					<xsl:attribute name="pid"><xsl:value-of select="eas:getSafeJSString($thisPrinciple/name)" /></xsl:attribute>
				</td>
				<td></td>
			</tr>
	...
	<xsl:template match="node()" mode="assessments">
		...
		"assessmentPos":"<xsl:choose><xsl:when test="current()/type='Control_Assessment'">
				<xsl:value-of select="(($thisScore/own_slot_value[slot_reference='enumeration_sequence_number']/value) div $allassessmentLevelsMax)" />
			</xsl:when><xsl:otherwise>
				<xsl:value-of select="(($thisScore/own_slot_value[slot_reference='enumeration_sequence_number']/value) div $allprincipleLevelsMax)" />
			</xsl:otherwise></xsl:choose>" }<xsl:if test="position()!=last()">,</xsl:if>
	</xsl:template>
</xsl:stylesheet>
