...
<xsl:stylesheet xpath-default-namespace="http://protege.stanford.edu/xml" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:functx="http://www.functx.com" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<!-- 11.02.2026 Converted to use strategyData API -->
	...
	<xsl:variable name="linkClasses" select="('Business_Objective', 'Business_Driver', 'Service_Quality', 'Service_Quality_Measure', 'Service_Quality_Value', 'Group_Actor', 'Individual_Actor','Enterprise_Strategic_Plan')"/>
 	...
 	<!-- Variables for supplemental data -->
	...
	<xsl:template name="BuildPage">
		...
		<html>
			...
			<body>
					...
					<div class="detail-outer-wrapper top-15">
						...
						<div class="detail-outer" style="background-color: var(--dark-grey);flex-basis:250px; color: #fff;">
							<div class="detail-outer-name obj-detail planClick">
								<xsl:attribute name="easid">{{this.id}}</xsl:attribute>
								{{{essRenderInstanceLink this 'Enterprise_Strategic_Plan'}}}
							</div>
							<div class="dash-subtitle text-white">{{this.description}}</div>
				...
				// Supplemental data for objectives (plans, projects, measures, owners)
				let supplementalData = {
					"objectiveDetails": [<xsl:apply-templates select="$allBusObjectives" mode="objSupplemental"/>]
				};
				
				let strategyData;
				let strategyDriversById = {};
				let strategyGoalsById = {};
				let strategyObjectivesById = {};
				
				$(document).ready(function () {
					// DEFINE APIS - the list of apis to call based on their data label
					const apiList=['strategyData'];
					
					async function executeFetchAndRender() {
						try {
							let responses = await fetchAndRenderData(apiList);
							strategyData = responses.strategyData;
							
							console.log('Strategy Data from API', strategyData);
							console.log('Supplemental Data from XSL', supplementalData);
							
							// Merge supplemental data into objectives
							mergeSupplementalData();
							
							// Create lookup objects
							strategyDriversById = Object.fromEntries(strategyData.drivers.map((d) => [d.id, d]));
							
							// Build goals lookup from nested structure
							strategyData.drivers.forEach(driver => {
								driver.goals.forEach(goal => {
									strategyGoalsById[goal.id] = goal;
								});
							});
							
							// Build objectives lookup from API objectives array
							strategyObjectivesById = Object.fromEntries(strategyData.objectives.map((o) => [o.id, o]));
							
							// Also collect all goals from the flat objectives array
							let allGoals = [];
							strategyData.drivers.forEach(driver => {
								driver.goals.forEach(goal => {
									if (!allGoals.find(g => g.id === goal.id)) {
										allGoals.push(goal);
									}
								});
							});
							
							// Create strategy map structure for templates
							let strategyMap = {
								strategy: {
									drivers: strategyData.drivers,
									goals: allGoals,
									objectives: strategyData.objectives
								}
							};
							
							console.log('Merged Strategy Map', strategyMap);
					...
					function mergeSupplementalData() {
						// Merge supplemental data (plans, projects, etc.) into objectives
						supplementalData.objectiveDetails.forEach(suppObj => {
							let apiObj = strategyData.objectives.find(o => o.id === suppObj.id);
							if (apiObj) {
								// Merge supplemental properties
								apiObj.plans = suppObj.plans || [];
								apiObj.busObjMeasures = suppObj.busObjMeasures || [];
								apiObj.busObjOrgOwners = suppObj.busObjOrgOwners || [];
								apiObj.busObjIndividualOwners = suppObj.busObjIndividualOwners || [];
							}
						});
						
						// Also merge into nested objectives within goals
						strategyData.drivers.forEach(driver => {
							driver.goals.forEach(goal => {
								if (goal.objectives) {
									goal.objectives.forEach(nestedObj => {
										let suppObj = supplementalData.objectiveDetails.find(s => s.id === nestedObj.id);
										if (suppObj) {
											nestedObj.plans = suppObj.plans || [];
											nestedObj.busObjMeasures = suppObj.busObjMeasures || [];
											nestedObj.busObjOrgOwners = suppObj.busObjOrgOwners || [];
											nestedObj.busObjIndividualOwners = suppObj.busObjIndividualOwners || [];
										}
									});
								}
							});
						...
						Handlebars.registerHelper('getObjectives', function (arg1, options) {
							...
							if (arg1.goals) {
								arg1.goals.forEach((e) => {
									oCount = oCount + (e.objectives ? e.objectives.length : 0);
								});
							}
							return oCount;
						});
						...
						Handlebars.registerHelper('getPlans', function (arg1, options) {
							...
							if (arg1.objectives) {
								arg1.objectives.forEach((e) => {
									pCount = pCount + (e.plans ? e.plans.length : 0);
								});
							}
							return pCount;
						});
						...
						Handlebars.registerHelper('getElements', function (arg1, options) {
							...
							if (arg1.plans) {
								arg1.plans.forEach((e) => {
									elCount = elCount + (e.plan2elements ? e.plan2elements.length : 0);
								});
							}
							return elCount;
						});
    ...
    <!-- Template to generate supplemental data for objectives -->
    <xsl:template match="node()" mode="objSupplemental">
