...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="allInitiatives" select="/node()/simple_instance[type = 'Initiative']"/>
	<xsl:variable name="allBusModels" select="/node()/simple_instance[type = 'Business_Model']"/>
	<xsl:variable name="allBusModelArchitectures" select="/node()/simple_instance[name = $allBusModels/own_slot_value[slot_reference = 'business_model_architecture']/value]"/>
	<xsl:variable name="allBusModelArchProdUsages" select="/node()/simple_instance[(type = 'Business_Model_Product_Usage') and (name = $allBusModelArchitectures/own_slot_value[slot_reference = 'business_model_architecture_elements']/value)]"/>
	<xsl:variable name="allBusModelProdTypes" select="/node()/simple_instance[name = $allBusModelArchProdUsages/own_slot_value[slot_reference = 'bmpu_used_product']/value]"/>
		...
		<html>
			<head>
				...
				<script>
					...
					var busCapModalContentTemplate, busProcessModalContentTemplate, orgModalContentTemplate, appServiceContentTemplate, appModalContentTemplate, stratPlanErrorsTemplate, roadmapExcelTemplate, dropdownTemplate;
					...
					function initInitiatives() {
						//where required, populate the initiative's object properties
						for (var i = 0; viewData.initiatives.length > i; i += 1) {
							var anInitiative = viewData.initiatives[i];
							if(anInitiative.businessModels == null) {
								var theBusModels = getObjectsByIds(viewData.businessModels, "id", anInitiative.busModelIds);
								anInitiative.businessModels = theBusModels;
							}
						}
					}
					...
					$(document).ready(function(){
						...
						$('#initiativeSelect').select2({
							theme: "bootstrap",
							placeholder: "Select an Initiave"
						});
						
						$('#busModelSelect').select2({
							theme: "bootstrap",
	...
	<xsl:template name="introPage">
		...
		<div class="appSelection dashboardPanel bg-offwhite bottom-30 pull-left">
			...
			<label for="initiativeSelect" class="impact">Initiative:</label>
			<select id="initiativeSelect" style="width: 300px;" class="right-30">
				<option/>
			</select>
			<label for="busModelSelect" class="left-30 impact">Business Model:</label>
			<select id="busModelSelect" disabled="true" style="width: 300px;" class="bottom-10">
				<option/>
			</select>
			<br/>
	...
	<xsl:template name="getReadOnlyJSON">
		{
			initiatives: [
				<xsl:apply-templates mode="getInittiatives" select="$allInitiatives"><xsl:sort select="own_slot_value[slot_reference = 'name']/value"/></xsl:apply-templates>
			],
			businessModels: [
				<xsl:apply-templates mode="getBusModels" select="$allBusModels"><xsl:sort select="own_slot_value[slot_reference = 'name']/value"/></xsl:apply-templates>
			],
			planningActions: [
	...
	<xsl:template match="node()" mode="getInittiatives">
	...
	<!-- Template for rendering the list of Planning Actions  -->
	<xsl:template match="node()" mode="getBusModels">
		<xsl:variable name="this" select="current()"/>
		
		<xsl:variable name="thisBusModelArchitectures" select="$allBusModelArchitectures[name = $this/own_slot_value[slot_reference = 'business_model_architecture']/value]"/>
		<xsl:variable name="thisBusModelArchProdUsages" select="$allBusModelArchProdUsages[name = $thisBusModelArchitectures/own_slot_value[slot_reference = 'business_model_architecture_elements']/value]"/>
