...
</xsl:call-template>","level":"<xsl:value-of select="$depth"/>","childrenCaps": [<xsl:if test="$depth &lt; 5"><xsl:apply-templates select="$subCaps" mode="busCapDetails"><xsl:with-param name="depth" select="$depth +1"/></xsl:apply-templates></xsl:if>],<xsl:call-template name="RenderSecurityClassificationsJSONForInstance"><xsl:with-param name="theInstance" select="current()"/></xsl:call-template>
