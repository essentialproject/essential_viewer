...
<xsl:template match="knowledge_base">
	...
	"busCaptoAppDetails":[<xsl:apply-templates select="$allBusCaps" mode="busCapDetailsApps"> </xsl:apply-templates>],
	"busCaptoAppScores":[<xsl:apply-templates select="$allBusCaps" mode="caps_apps_map"> </xsl:apply-templates>],
 	"busFitValues":[{"title":"Key", "values":[<xsl:apply-templates select="$BFValues" mode="fitValues"><xsl:sort select="current()/own_slot_value[slot_reference='service_quality_value_score']/value" order="ascending"/></xsl:apply-templates>]}],
	"techFitValues":[{"title":"Key", "values":[<xsl:apply-templates select="$AFValues" mode="fitValues"><xsl:sort select="current()/own_slot_value[slot_reference='service_quality_value_score']/value" order="ascending"/></xsl:apply-templates>]}],
	"lifecycles":[{"title":"Lifecycles","values":[<xsl:apply-templates select="$allLifecycleStatus" mode="lifes"><xsl:sort select="own_slot_value[slot_reference='enumeration_sequence_number']/value" order="ascending"></xsl:sort></xsl:apply-templates>]}],
...
<xsl:template mode="busCapDetails" match="node()">
		...
		<xsl:variable name="thisappsWithCapsPreFilter" select="$thisrelevantAppsviaAPR union $thisrelevantApps"></xsl:variable>  
		<xsl:variable name="thisappsWithCaps" select="$thisappsWithCapsPreFilter[own_slot_value[slot_reference='lifecycle_status_application_provider']/value=$allLifecycleStatustoShow/name]"></xsl:variable>  
	...
	"childrenCaps": [<xsl:if test="$depth &lt; 10"><xsl:apply-templates select="$subCaps" mode="busCapDetails"><xsl:with-param name="depth" select="$depth +1"/></xsl:apply-templates></xsl:if>], 
	"apps":[ <xsl:apply-templates select="$thisappsWithCaps" mode="applications"></xsl:apply-templates>], 
...
<xsl:variable name="thisrelevantbusProcs" select="$relevantBusProcs[name=$allRelevantPhysProcesses/own_slot_value[slot_reference = 'implements_business_process']/value]"></xsl:variable>
<xsl:variable name="thisrelevantBusCaps" select="$allBusCaps[name=$thisrelevantbusProcs/own_slot_value[slot_reference = 'realises_business_capability']/value]"></xsl:variable>
 
<xsl:variable name="appStaticUsages" select="$allAppStaticUsages[own_slot_value[slot_reference = 'static_usage_of_app_provider']/value = current()/name]"/>
...
<xsl:variable name="appOutboundDepCount" select="eas:get_outbound_int_count($allCurrentApps)"/>
		...
		"inboundIntegrationList":[<xsl:for-each select="$appInboundStaticApps">{"name":"<xsl:call-template name="RenderMultiLangInstanceName"><xsl:with-param name="theSubjectInstance" select="current()"/>
		<xsl:with-param name="isForJSONAPI" select="true()"/>
		...
		"outboundIntegrationList":[<xsl:for-each select="$appOutboundStaticApps">{"name":"<xsl:call-template name="RenderMultiLangInstanceName">
			<xsl:with-param name="theSubjectInstance" select="current()"/>
			<xsl:with-param name="isForJSONAPI" select="true()"/>
		</xsl:call-template>", "id":"<xsl:value-of select="eas:getSafeJSString(current()/name)"/>"}<xsl:if test="not(position() = last())">,</xsl:if></xsl:for-each>],	
		...
		"codebase":"<xsl:value-of select="$thisCodebase/own_slot_value[slot_reference='name']/value"/>",
		"codebaseColour":"<xsl:choose><xsl:when test="$thisCodebaseStyle/own_slot_value[slot_reference = 'element_style_colour']/value"><xsl:value-of select="$thisCodebaseStyle/own_slot_value[slot_reference = 'element_style_colour']/value"/></xsl:when><xsl:otherwise>#48a3db</xsl:otherwise></xsl:choose>",
		"codebaseText":"<xsl:choose><xsl:when test="$thisCodebaseStyle/own_slot_value[slot_reference = 'element_style_text_colour']/value"><xsl:value-of select="$thisCodebaseStyle/own_slot_value[slot_reference = 'element_style_text_colour']/value"/></xsl:when><xsl:otherwise>#ffffff</xsl:otherwise></xsl:choose>",
		
		"appDelivery":"<xsl:value-of select="$appDelivery/own_slot_value[slot_reference='name']/value"/>",
		"deliveryColour":"<xsl:choose><xsl:when test="$thisDepStyle/own_slot_value[slot_reference = 'element_style_colour']/value"><xsl:value-of select="$thisDepStyle/own_slot_value[slot_reference = 'element_style_colour']/value"/></xsl:when><xsl:otherwise>#2fa191</xsl:otherwise></xsl:choose>",
		"deliveryText":"<xsl:choose><xsl:when test="$thisDepStyle/own_slot_value[slot_reference = 'element_style_text_colour']/value"><xsl:value-of select="$thisDepStyle/own_slot_value[slot_reference = 'element_style_text_colour']/value"/></xsl:when><xsl:otherwise>#ffffff</xsl:otherwise></xsl:choose>",

		"lifecycleColor":"<xsl:choose><xsl:when test="$thisStyle/own_slot_value[slot_reference = 'element_style_colour']/value"><xsl:value-of select="$thisStyle/own_slot_value[slot_reference = 'element_style_colour']/value"/></xsl:when><xsl:otherwise>#ebdd8f</xsl:otherwise></xsl:choose>",
		"lifecycleText":"<xsl:choose><xsl:when test="$thisStyle/own_slot_value[slot_reference = 'element_style_text_colour']/value"><xsl:value-of select="$thisStyle/own_slot_value[slot_reference = 'element_style_text_colour']/value"/></xsl:when><xsl:otherwise>#000000</xsl:otherwise></xsl:choose>",
		
		"lifecycleStatus":"<xsl:value-of select="$allLifecycleStatus[name=current()/own_slot_value[slot_reference='lifecycle_status_application_provider']/value]/own_slot_value[slot_reference='name']/value"/>",
	
		"lifecycle":"<xsl:value-of select="$appLifecycle/own_slot_value[slot_reference = 'enumeration_value']/value"/>",
		"processes":[<xsl:for-each select="$thisrelevantbusProcs">{
			"id": "<xsl:value-of select="eas:getSafeJSString(current()/name)"></xsl:value-of>", 
			"link": "<xsl:call-template name="RenderInstanceLinkForJS">
					<xsl:with-param name="isForJSONAPI" select="true()"/>
					<xsl:with-param name="theSubjectInstance" select="current()"></xsl:with-param>
				</xsl:call-template>"}<xsl:if test="not(position() = last())"><xsl:text>,</xsl:text></xsl:if></xsl:for-each>],
		"physicalProcesses":[<xsl:for-each select="$allRelevantPhysProcesses">{
			"id": "<xsl:value-of select="eas:getSafeJSString(current()/name)"></xsl:value-of>", 
			"link": "<xsl:call-template name="RenderInstanceLinkForJS">
					<xsl:with-param name="isForJSONAPI" select="true()"/>
					<xsl:with-param name="theSubjectInstance" select="current()"></xsl:with-param>
				</xsl:call-template>"}<xsl:if test="not(position() = last())"><xsl:text>,</xsl:text></xsl:if></xsl:for-each>],
		"capabilities":[<xsl:for-each select="$thisrelevantBusCaps">{
			"id": "<xsl:value-of select="eas:getSafeJSString(current()/name)"></xsl:value-of>", 
			"link": "<xsl:call-template name="RenderInstanceLinkForJS">
					<xsl:with-param name="isForJSONAPI" select="true()"/>
					<xsl:with-param name="theSubjectInstance" select="current()"></xsl:with-param>
...
<xsl:template match="node()" mode="busCapDetailsApps">
	...
	"apps":[<xsl:apply-templates select="$thisappsWithCaps" mode="applications"></xsl:apply-templates>]}<xsl:if test="not(position() = last())"><xsl:text>,
	</xsl:text></xsl:if>
</xsl:template> 
...
<xsl:template match="node()" mode="caps_apps_map">
        ...
        <xsl:variable name="sthisrelevantAppsviaAPR" select="$relevantAppsviaAPR[name = $sthisrelevantAppProRoles/own_slot_value[slot_reference = 'role_for_application_provider']/value]"/>
		<xsl:variable name="sthisappsWithCapsPreFilter" select="$sthisrelevantApps union $sthisrelevantAppsviaAPR"/>
		<xsl:variable name="sthisappsWithCaps" select="$sthisappsWithCapsPreFilter[own_slot_value[slot_reference='lifecycle_status_application_provider']/value=$allLifecycleStatustoShow/name]"></xsl:variable>  
        ...
        <xsl:variable name="subthisrelevantApps" select="$relevantAppsviaAPR[name = $subthisrelevantAppProRoles/own_slot_value[slot_reference = 'role_for_application_provider']/value]"/>
		...
		<xsl:variable name="thisApp" select="$subthisrelevantApps union $subthisrelevantAppsDirect"/>	
      ...
      {"name":"<xsl:choose><xsl:when test="$subthisrelevantApps/name"><xsl:call-template name="RenderMultiLangInstanceName">
                <xsl:with-param name="theSubjectInstance" select="$subthisrelevantApps"/>
                <xsl:with-param name="isRenderAsJSString" select="true()"/>
            </xsl:call-template></xsl:when><xsl:otherwise><xsl:call-template name="RenderMultiLangInstanceName">
                <xsl:with-param name="theSubjectInstance" select="$subthisrelevantAppsDirect"/>
                <xsl:with-param name="isRenderAsJSString" select="true()"/>
		   </xsl:call-template></xsl:otherwise></xsl:choose><!--<xsl:value-of select="$subthisrelevantApps/own_slot_value[slot_reference='name']/value"/><xsl:value-of select="eas:getSafeJSString($subthisrelevantAppsDirect/own_slot_value[slot_reference='name']/value)"/>-->",
		 "criticality":"<xsl:value-of select="$criticalityStatus[name=$subthisrelevantApps/own_slot_value[slot_reference='ap_business_criticality']/value]/own_slot_value[slot_reference='enumeration_value']/value"/>",
		 "orgUserIds": [<xsl:for-each select="$thisOrgUserIds">"<xsl:value-of select="."/>"<xsl:if test="not(position() = last())"><xsl:text>,</xsl:text></xsl:if></xsl:for-each>],  
		 "capappid":"<xsl:value-of select="eas:getSafeJSString($this)"/><xsl:choose><xsl:when test="$subthisrelevantApps/name"><xsl:value-of select="eas:getSafeJSString($subthisrelevantApps/name)"/></xsl:when><xsl:otherwise><xsl:value-of select="eas:getSafeJSString($subthisrelevantAppsDirect/name)"/></xsl:otherwise></xsl:choose>",
		 "id":"<xsl:choose><xsl:when test="$subthisrelevantApps/name"><xsl:value-of select="eas:getSafeJSString($subthisrelevantApps/name)"/></xsl:when><xsl:otherwise><xsl:value-of select="eas:getSafeJSString($subthisrelevantAppsDirect/name)"/></xsl:otherwise></xsl:choose>",
        "busScore":"<xsl:value-of select="$busScore"/>", 
        "busFit":"<xsl:value-of select="$busFit"/>",
		"busFitVal":"<xsl:value-of select="$thisBFValues/name"/>",
        "techFit":"<xsl:value-of select="$appFit"/>",
		"appScoreOriginal":"<xsl:value-of select="$appScore"/>",
       	<!--"count":"<xsl:value-of select="position()"/><xsl:text>/</xsl:text><xsl:value-of select="count($allProcs)"/>",-->
        "appScore":"<xsl:value-of select="$appScore"/>","techFitVal":"<xsl:value-of select="$thisAFValues/name"/>", "lifecycle":"<xsl:value-of select="$appLifecycle/own_slot_value[slot_reference = 'enumeration_value']/value"/>","lifecycleColor":"<xsl:choose><xsl:when test="$thisStyle/own_slot_value[slot_reference = 'element_style_colour']/value"><xsl:value-of select="$thisStyle/own_slot_value[slot_reference = 'element_style_colour']/value"/></xsl:when><xsl:otherwise>#ebdd8f</xsl:otherwise></xsl:choose>","lifecycleText":"<xsl:choose><xsl:when test="$thisStyle/own_slot_value[slot_reference = 'element_style_text_colour']/value"><xsl:value-of select="$thisStyle/own_slot_value[slot_reference = 'element_style_text_colour']/value"/></xsl:when><xsl:otherwise>#000000</xsl:otherwise></xsl:choose>",<xsl:call-template name="RenderSecurityClassificationsJSONForInstance"><xsl:with-param name="theInstance" select="current()"/></xsl:call-template>
	}<xsl:if test="not(position() = last())"><xsl:text>,</xsl:text></xsl:if>
			</xsl:if>
        </xsl:for-each>]
	...
	<xsl:template match="node()" mode="apps">
        <xsl:variable name="thisApp" select="current()/name"></xsl:variable>
		 <xsl:variable name="thisBusCapName">
			<xsl:call-template name="RenderMultiLangInstanceName">
				<xsl:with-param name="theSubjectInstance" select="current()"/>
			</xsl:call-template>
	...
	<xsl:variable name="thisAppLink">
			<xsl:call-template name="RenderInstanceLinkForJS">
				<xsl:with-param name="theSubjectInstance" select="current()"/>
			</xsl:call-template>
		</xsl:variable>  
		{"id":"<xsl:value-of select="eas:getSafeJSString(current()/name)"/>",
		"name":"<xsl:value-of select="$thisApp"/>",
		"link":"<xsl:value-of select="$thisAppLink"/>",
		"description":"<xsl:value-of select="$thisAppDescription"/>",
		"codebase":"<xsl:value-of select="$codebase[name=current()/own_slot_value[slot_reference='ap_codebase_status']/value]/own_slot_value[slot_reference='name']/value"/>",
		"appdelivery":"<xsl:value-of select="$delivery[name=current()/own_slot_value[slot_reference='ap_delivery_model']/value]/own_slot_value[slot_reference='name']/value"/>",
		"lifecycle":"<xsl:value-of select="$allLifecycleStatus[name=current()/own_slot_value[slot_reference='lifecycle_status_application_provider']/value]/own_slot_value[slot_reference='name']/value"/>"}<xsl:if test="position()!=last()">,</xsl:if>
