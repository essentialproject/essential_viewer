		...
		<html>
			...
			-->
				...
				<script id="appList-template" type="text/x-handlebars-template">
					 	<span id="capsId"><xsl:attribute name="easid">{{this.cap}}</xsl:attribute>{{this.caps}}</span>
						{{#each this.apps}}
							<div class="appBox">
				...
				<script id="app-template" type="text/x-handlebars-template">
					<div class="row">
	            		<div class="col-sm-8">
	            			<h4 class="text-normal strong inline-block right-30">{{#essRenderInstanceLinkMenuOnly this 'Composite_Application_Provider'}}{{/essRenderInstanceLinkMenuOnly}}</h4>
...
var redrawView=function(){
	...
	let scopedApps = essScopeResources(apps, [appOrgScopingDef, geoScopingDef]);
	let appsToShow=[];
	...
	appMod.then((d)=>{
		...
		console.log(workingArrayAppsCaps)
...
function getApps(capid){
	let thisappList = workingArrayAppsCaps.filter(function (d)
	{
		return d.id == capid;
	});
	appsToShow['applications']=scopedApps.resources;
 
	let filteredApps = thisappList[0].apps.filter((id) => scopedApps.resourceIds.includes(id));
