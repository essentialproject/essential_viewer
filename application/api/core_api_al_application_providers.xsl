...
<!--		<xsl:variable name="thisallActors" select="$allActors[name=$thisAppOrgUser2Roles/own_slot_value[slot_reference = 'act_to_role_from_actor']/value]"/>-->
      ...
      "service":[<xsl:for-each select="$thisAppProviderRoles"><xsl:variable name="thisSvc" select="$thisAppServices[own_slot_value[slot_reference='provided_by_application_provider_roles']/value=current()/name]"/>{"id":"<xsl:value-of select="eas:getSafeJSString(current()/name)"/>","name":"<xsl:call-template name="RenderInstanceLink"><xsl:with-param name="isForJSONAPI" select="true()"/><xsl:with-param name="theSubjectInstance" select="$thisSvc"/></xsl:call-template>"}<xsl:if test="not(position()=last())">, </xsl:if></xsl:for-each>],
			"appOrgUsers": [<xsl:for-each select="$thsAppUsersIn"><xsl:variable name="thisActors" select="$thisarl[name=current()]"/>{"id":"<xsl:value-of select="eas:getSafeJSString(.)"/>","name":"<xsl:value-of select="$thisActors/own_slot_value[slot_reference = 'name']/value"/>","link":"<xsl:call-template name="RenderInstanceLink"><xsl:with-param name="isForJSONAPI" select="true()"/><xsl:with-param name="theSubjectInstance" select="$thisActors"/></xsl:call-template>"}<xsl:if test="not(position()=last())">, </xsl:if></xsl:for-each>]
