...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
    ...
    <xsl:variable name="applicationServices" select="/node()/simple_instance[type=('Application_Service','Composite_Application_Service')]"/> 
    <xsl:key name="applicationProvidersKey" match="/node()/simple_instance[type=('Application_Provider','Composite_Application_Provider','Application_Provider_Interface')]" use="type"/>
    <xsl:variable name="applicationProviders" select="key('applicationProvidersKey', ('Application_Provider','Composite_Application_Provider','Application_Provider_Interface'))"/>
	<xsl:key name="applicationProviderRolesKey" match="/node()/simple_instance[type=('Application_Provider_Role')]" use="type"/>
	<xsl:variable name="applicationProviderRoles" select="key('applicationProviderRolesKey','Application_Provider_Role')"/>
	...
	<xsl:key name="allTechBuildsNameKey" match="/node()/simple_instance[supertype='Technology_Provider']" use="name"/>
  ...
  <!-- 	
	...
	<xsl:key name="appSvc_key" match="/node()/simple_instance[type=('Application_Service','Composite_Application_Service')]" use="own_slot_value[slot_reference = 'realises_application_capabilities']/value"/>
	...
	<xsl:template match="node()" mode="appTech">
		...
		<!--	<xsl:variable name="environment" select="$allAppDeployments[own_slot_value[slot_reference = 'application_provider_deployed']/value=current()/name]"/>
			...
			<xsl:variable name="aTechBuild" select="key('allTechBuildsNameKey', $environment/own_slot_value[slot_reference = 'application_deployment_technical_arch']/value)"/>
