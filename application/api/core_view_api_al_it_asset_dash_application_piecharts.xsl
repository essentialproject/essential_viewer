...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="allAppProviders" select="/node()/simple_instance[type = ('Application_Provider','Composite_Application_Provider')]"/>
 	<xsl:variable name="appOrgUser" select="/node()/simple_instance[type = 'Group_Business_Role'][own_slot_value[slot_reference='name']/value=('Application Organisation User','Application User')]"/>
	<xsl:variable name="a2r" select="/node()/simple_instance[type = 'ACTOR_TO_ROLE_RELATION'][own_slot_value[slot_reference='act_to_role_to_role']/value=$appOrgUser/name]"/>
  ...
  <!--  <xsl:variable name="allRoadmapInstances" select="$allAppProviders"/>
	...
	<xsl:template match="knowledge_base">
		{
			"codebases": [
				<xsl:apply-templates select="$codebase" mode="RenderCodebasePieData"/>
			],
			"delivery": [
	...
	<xsl:template mode="RenderCodebasePieData" match="node()">
	 	<xsl:variable name="thisAppProviders" select="$allAppProviders[own_slot_value[slot_reference='ap_codebase_status']/value=current()/name]"/>
     ...
     
		{
			...
			"count": <xsl:value-of select="count($thisAppProviders)"></xsl:value-of>,
			"piecount": <xsl:value-of select="count($thisAppProviders)"></xsl:value-of>,
		    ...
		    "orgsUsage":[<xsl:apply-templates select="$thisAppProviders" mode="renderOrgData"/>]
	...
	<xsl:template mode="RenderDeliveryPieData" match="node()">
	 	<xsl:variable name="thisAppProviders" select="$allAppProviders[own_slot_value[slot_reference='ap_delivery_model']/value=current()/name]"/>
		 <xsl:variable name="thisColour" select="$lifecycleColour[own_slot_value[slot_reference='style_for_elements']/value=current()/name]"/>
		{
			"id": "<xsl:value-of select="eas:getSafeJSString(current()/name)"/>",
		 	"name":"<xsl:call-template name="RenderMultiLangInstanceName">
