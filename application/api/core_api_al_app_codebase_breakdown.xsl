...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="allAppCodbases" select="/node()/simple_instance[name = $allAppProviders/own_slot_value[slot_reference = 'ap_codebase_status']/value]"/>

	<xsl:template match="knowledge_base">
		{
			"applications": [
				<xsl:apply-templates mode="RenderCodebaseProportion" select="$allAppCodbases">
	...
	<xsl:template mode="RenderCodebaseProportion" match="node()">
		<xsl:variable name="thisModelCount" select="count($allAppProviders[own_slot_value[slot_reference = 'ap_codebase_status']/value = current()/name])"/>
		...
		{
		"codebase": "<xsl:value-of select="$thisLabel"/>",
