...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="allTaxonomy" select="/node()/simple_instance[type='Taxonomy']"/>
	...
	<xsl:key name="domainKey" match="/node()/simple_instance[type=('Business_Domain')]" use="name"/>
	...
	<xsl:variable name="allTechProviders" select="/node()/simple_instance[supertype='Technology_Provider']"/>
	...
	<xsl:variable name="thisApplicationServices" select="key('appSvc_key',current()/name)"/> 												
	  {
			...
			"SupportedBusCapability":[<xsl:for-each select="$supportedBusCaps">
					{"id":"<xsl:value-of select="eas:getSafeJSString(current()/name)"/>",
					"name":"<xsl:call-template name="RenderMultiLangInstanceName"><xsl:with-param name="theSubjectInstance" select="current()"/><xsl:with-param name="isForJSONAPI" select="true()"/></xsl:call-template>"}<xsl:if test="position()!=last()">,</xsl:if></xsl:for-each>],	
			"ReferenceModelLayer":"<xsl:call-template name="RenderMultiLangInstanceName"><xsl:with-param name="theSubjectInstance" select="$refLayer[1]"/><xsl:with-param name="isForJSONAPI" select="true()"/></xsl:call-template>",
							...
							-->
								...
								{"id":"<xsl:value-of select="eas:getSafeJSString(current()/name)"/>",
								 ...
								 "org":"<xsl:call-template name="RenderMultiLangInstanceName">
										<xsl:with-param name="theSubjectInstance" select="$thisAppStandardOrg[1]"/>
