...
<xsl:stylesheet xpath-default-namespace="http://protege.stanford.edu/xml" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:functx="http://www.functx.com" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<!-- 19.01.2012	JWC Catalogue of applications to select for Application Interface Change Analysis -->
	...
	<xsl:variable name="targetReport" select="/node()/simple_instance[(type = 'Report') and (own_slot_value[slot_reference = 'name']/value = 'Core: Application Change Analysis - Application Interface')]"/>
					...
					<div class="row">
								...
								<div class="page-header">
									<h1>
										...
										<span class="text-darkgrey">
											<xsl:value-of select="eas:i18n('Application Interface Change Analysis Catalogue')"/>
										</span>
									</h1>
								</div>
							</div>
						...
						<div id="sectionAbout" class="hidden">
							<div class="col-xs-12">
								...
								<div>
									<p><xsl:value-of select="eas:i18n('This view shows all the Applications contained in the repository. Select the Application from this list to view its Application Interface Change Analysis')"/>.</p>
								</div>
							</div>
							<hr/>
							<div class="clear"/>
					...
					<div class="row">
						<div id="sectionCatalogue">
							<div class="col-xs-12">
								...
								<div>
									<h2 class="text-primary">
										<xsl:value-of select="eas:i18n('Select Application for Interface Change Analysis')"/>
									</h2>
								</div>
								<div>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:if test="count($orphanApps) > 0">
					<h3>
						<xsl:value-of select="eas:i18n('Others (No Parent Application)')"/>
					</h3>
	...
	<xsl:template match="node()" mode="Application_Providers">
		...
		<li>
			<xsl:call-template name="RenderInstanceLink">
				<xsl:with-param name="theSubjectInstance" select="$thisAP"/>
			...
			<!--<a>
				<xsl:call-template name="RenderLinkHref">
					...
					<xsl:with-param name="theXML" select="$reposXML" />
					<xsl:with-param name="theXSL">application/core_al_app_gap_app_interface_app.xsl</xsl:with-param>
					<xsl:with-param name="theParam4" select="$param4" />
					<!-\- pass the id of the taxonomy term used for scoping as parameter 4-\->
					<xsl:with-param name="theHistoryLabel">Application Change Analysis - Application Interface for <xsl:value-of select="$ap_name" /></xsl:with-param>
				</xsl:call-template>
