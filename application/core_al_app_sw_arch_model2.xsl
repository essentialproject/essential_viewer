...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:functx="http://www.functx.com">
	...
	<xsl:variable name="orphanSWCompUsages" select="$appSWCompUsages[not(own_slot_value[slot_reference = 'usage_of_software_component']/value = $appSWComps/name)]"/>
	<xsl:variable name="orphanInfoRepUsages" select="$appInfoRepUsages[not(own_slot_value[slot_reference = 'software_usage_of_info_rep']/value = $appInfoReps/name)]"/>

	<xsl:variable name="orphanUsages" select="$orphanSWCompUsages union $orphanInfoRepUsages"/>
				...
				<div class="container-fluid">
					<div class="row">
						...
						<div class="col-xs-12">
							...
							<xsl:choose>
								<xsl:when test="count($orphanUsages) > 0">
									<h3>Modelling Issue</h3>
									<p>
										<xsl:value-of select="eas:i18n('The following usages in the software architecture model for ')"/><xsl:call-template name="RenderInstanceLink"><xsl:with-param name="theSubjectInstance" select="$appProvNode"/></xsl:call-template><xsl:value-of select="eas:i18n(' have not been linked to their associated Software or Information Components:')"/>
									</p>
									<xsl:if test="$orphanSWCompUsages">
										<p><strong>Software Components</strong></p>
										<ul>
											<xsl:for-each select="$orphanSWCompUsages">
												<xsl:variable name="usageLabel" select="current()/own_slot_value[slot_reference = 'software_architecture_label']/value"/>
												<xsl:choose>
													<xsl:when test="string-length($usageLabel) > 0">
														<li><xsl:value-of select="$usageLabel"/></li>
													</xsl:when>
													<xsl:otherwise>
														<li><xsl:value-of select="current()/own_slot_value[slot_reference = 'name']/value"/></li>
													</xsl:otherwise>
												</xsl:choose>
											</xsl:for-each>
										</ul>
									</xsl:if>
									<xsl:if test="$orphanInfoRepUsages">
										<p><strong>Information Components</strong></p>
										<ul>
											<xsl:for-each select="$orphanInfoRepUsages">
												<xsl:variable name="usageLabel" select="current()/own_slot_value[slot_reference = 'software_architecture_label']/value"/>
												<xsl:choose>
													<xsl:when test="string-length($usageLabel) > 0">
														<li><xsl:value-of select="$usageLabel"/></li>
								...
								<xsl:when test="count($appSWComps union $appInfoReps) > 0">
	...
	<xsl:template name="modelScript">
		...
		<xsl:if test="(count($appSWComps union $appInfoReps) > 0) and (count($orphanUsages) = 0)">
			<script>
