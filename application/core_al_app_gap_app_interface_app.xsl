...
<xsl:stylesheet xpath-default-namespace="http://protege.stanford.edu/xml" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:functx="http://www.functx.com" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<!-- 19.01.2012	JWC	Updated for Application Interface Change Analysis -->
	...
	<xsl:variable name="gapReportLabel">
		<xsl:value-of select="eas:i18n('Application Change Analysis - Application Interface')"/>
	</xsl:variable>

	<!-- CHANGE THIS TEXT TO LIST THE TYPES OF ELEMENTS IN SCOPE FOR THE Change Analysis (USED IN DESCRIPTIONS) -->
	<xsl:variable name="gapReportElementDesc" select="'Application Interfaces'"/>
	...
	<xsl:template name="PrintElementTypePrefix">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Application_Provider'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Application')"/>
				</span>
				<br/>
			</xsl:when>
			<xsl:when test="$elementType = ':APU-TO-APU-STATIC-RELATION'">
				<span class="small normal">
	...
	<xsl:template name="PrintElementDisplayLabel">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Application_Provider'">
				<!--<xsl:value-of select="$theElement/own_slot_value[slot_reference='name']/value" />-->
				<xsl:call-template name="RenderInstanceLink">
					<xsl:with-param name="theSubjectInstance" select="$theElement"/>
					<xsl:with-param name="theXML" select="$reposXML"/>
					<xsl:with-param name="viewScopeTerms" select="$viewScopeTerms"/>
					<xsl:with-param name="anchorClass" select="'textColor7'"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="$elementType = ':APU-TO-APU-STATIC-RELATION'">
				<!--<xsl:value-of select="$theElement/own_slot_value[slot_reference=':relation_label']/value" />-->
				<xsl:call-template name="RenderInstanceLink">
					<xsl:with-param name="theSubjectInstance" select="$theElement"/>
					<xsl:with-param name="theXML" select="$reposXML"/>
					<xsl:with-param name="viewScopeTerms" select="$viewScopeTerms"/>
	...
	<xsl:template name="BuildPage">
		...
		<!-- Get all the Usages of the selected Application -->
		<xsl:variable name="applicationUsages" select="/node()/simple_instance[own_slot_value[slot_reference = 'static_usage_of_app_provider']/value = $param1]"/>
		<!-- All gap elements are all the :APU-TO-APU-STATIC-RELATIONS in which the usages appear in the :TO or :FROM slots -->
		<xsl:variable name="allGapElements" select="/node()/simple_instance[(type = ':APU-TO-APU-STATIC-RELATION') and ((own_slot_value[slot_reference = ':TO']/value = $applicationUsages/name) or (own_slot_value[slot_reference = ':FROM']/value = $applicationUsages/name))]"/>
		...
		<xsl:variable name="currentStateElements" select="$allGapElements[name = $startArchState/own_slot_value[slot_reference = 'arch_state_application_logical']/value]"/>
		<xsl:variable name="targetStateElements" select="$allGapElements[name = $targetArchState/own_slot_value[slot_reference = 'arch_state_application_logical']/value]"/>
