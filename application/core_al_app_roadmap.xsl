	...
	<xsl:template match="knowledge_base">
		...
		<xsl:choose>
			<xsl:when test="string-length($param4) > 0">
				<xsl:variable name="roadmapConstant" select="/node()/simple_instance[own_slot_value[slot_reference = 'report_constant_short_name']/value = 'Application_Roadmap']"/>
				...
				<xsl:variable name="genericPageLabel">
					<xsl:value-of select="eas:i18n('Application Roadmap')"/>
				</xsl:variable>
			...
			<xsl:otherwise>
				<xsl:variable name="roadmapConstant" select="/node()/simple_instance[(own_slot_value[slot_reference = 'report_constant_short_name']/value = 'Application_Roadmap')][1]"/>
				<xsl:variable name="roadmap" select="/node()/simple_instance[name = $roadmapConstant/own_slot_value[slot_reference = 'report_constant_ea_elements']/value]"/>
				<xsl:call-template name="BuildPage">
					<xsl:with-param name="roadmap" select="$roadmap"/>
				</xsl:call-template>
						...
						<xsl:for-each select="$roadmapMilestones">
							...
							<xsl:variable name="linkURL">
								<xsl:call-template name="RenderFullLinkText">
									...
									<xsl:with-param name="theXSL" select="'application/core_al_app_milestone_summary.xsl'"/>
									<xsl:with-param name="theInstanceID" select="current()/name"/>
									<xsl:with-param name="theXML">reportXML.xml</xsl:with-param>
									<xsl:with-param name="theHistoryLabel">Milestone Summary - <xsl:value-of select="$currentArchStateName"/></xsl:with-param>
					...
					<xsl:for-each select="$strategicPlans">
						...
						<xsl:variable name="linkURL">
							<xsl:call-template name="RenderFullLinkText">
								...
								<xsl:with-param name="theInstanceID" select="current()/name"/>
								<xsl:with-param name="theXML">reportXML.xml</xsl:with-param>
								<xsl:with-param name="theHistoryLabel">Strategic Plan Summary - <xsl:value-of select="current()/own_slot_value[slot_reference = 'name']/value"/></xsl:with-param>
								<xsl:with-param name="theParam4" select="$param4"/> <!-- pass the id of the taxonomy term used for scoping as parameter 4-->
							</xsl:call-template>
						</xsl:variable>
					...
					<div class="row">
						...
						<div class="col-xs-12">
							...
							<div class="content-section">
								...
								<xsl:if test="count($roadmap) = 0">
									<p>
										<em>No Application Roadmap defined</em>
									</p>
								</xsl:if>
								<xsl:if test="count($roadmap) &gt; 0">
