...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:functx="http://www.functx.com">
	...
	<xsl:variable name="infoRepColour">hsla(344, 100%, 89%, 1)</xsl:variable>
		...
		<xsl:if test="count($appSWComps) > 0">
			<script>
					...
					var paper = new joint.dia.Paper({
				        ...
				        height: <xsl:value-of select="($objectHeight + 30) * count($allAppSWComps)"/>,
				        gridSize: 1,
				        model: graph
				    });
					...
					joint.shapes.custom.Cluster = joint.shapes.basic.Rect.extend({
					    ...
					    defaults: joint.util.deepSupplement({
					        type: 'custom.Cluster',
					        attrs: {
					            rect: { rx: "15", ry: "15", fill: '#E67E22', stroke: '#D35400', 'stroke-width': 5 },
					            text: { 
					            	fill: 'white',
					            	'ref-x': .5,
		                        	'ref-y': .4
					...
					joint.shapes.custom.InfoRep = joint.shapes.basic.Circle.extend({
						markup: '<g class="rotatable"><g class="scalable"><circle/></g><text/></g>',
					    defaults: joint.util.deepSupplement({
					        type: 'custom.InfoRep',
					        attrs: {
					            circle: {fill: '#E67E22', stroke: '#D35400', 'stroke-width': 5 },
					            text: { 
					            	fill: 'white',
					            	'ref-x': .5,
		                        	'ref-y': .4
	...
	<xsl:template mode="RenderSWCompDefinition" match="node()">
		...
		<xsl:choose>
			<xsl:when test="current()/name = $appInfoReps/name"><xsl:value-of select="$nodeListName"/>: new joint.shapes.custom.InfoRep({ position: { x: 100, y: 20 }, size: { width: <xsl:value-of select="$objectWidth"/>, height: <xsl:value-of select="$objectHeight"/> }, attrs: {circle: {'stroke-width': <xsl:value-of select="$objectStrokeWidth"/>, fill: '<xsl:value-of select="$infoRepColour"/>', stroke: '<xsl:value-of select="$objectOutlineColour"/>'<!--, rx: 5, ry: 5--> }, text: { text: <xsl:value-of select="$nameVariable"/>, fill: '<xsl:value-of select="$objectTextColour"/>', 'font-weight': 'bold', 'font-size': 18 }} })</xsl:when>
			<xsl:otherwise><xsl:value-of select="$nodeListName"/>: new joint.shapes.custom.Cluster({ position: { x: 100, y: 20 }, size: { width: <xsl:value-of select="$objectWidth"/>, height: <xsl:value-of select="$objectHeight"/> }, attrs: {rect: {'stroke-width': <xsl:value-of select="$objectStrokeWidth"/>, fill: '<xsl:value-of select="$objectColour"/>', stroke: '<xsl:value-of select="$objectOutlineColour"/>'<!--, rx: 5, ry: 5--> }, text: { text: <xsl:value-of select="$nameVariable"/>, fill: '<xsl:value-of select="$objectTextColour"/>', 'font-weight': 'bold', 'font-size': 18 }} })</xsl:otherwise>
	...
	<xsl:template name="legend">
		<div class="keyContainer">
			...
			<div class="keySampleWide">
				<xsl:attribute name="style">background-color: <xsl:value-of select="$objectColour"/>;</xsl:attribute>
			</div>
			<div class="keySampleLabel">Software Component</div>
			<div class="keySampleWide">
				<xsl:attribute name="style">background-color: <xsl:value-of select="$infoRepColour"/>;</xsl:attribute>
			</div>
			<div class="keySampleLabel">Data Store</div>
		</div>
	</xsl:template>
