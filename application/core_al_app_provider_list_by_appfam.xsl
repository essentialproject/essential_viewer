...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="linkClasses" select="('Application_Provider', 'Composite_Application_Provider', 'Application_Family')"/>
	
	
	<xsl:variable name="allInstances" select="/node()/simple_instance[supertype = $instanceClassName or type = $instanceClassName]"/>
	<xsl:variable name="allInstanceCategories" select="/node()/simple_instance[name = $allInstances/own_slot_value[slot_reference = 'type_of_application']/value]"/>
	<xsl:variable name="orphanInstances" select="$allInstances[not(own_slot_value[slot_reference = 'type_of_application']/value)]"/>
	<xsl:variable name="categorySlot">type_of_application</xsl:variable>
	
	<xsl:variable name="catalogueTitle" select="eas:i18n('Application Catalogue by Family')"/>
	<xsl:variable name="catalogueSectionTitle" select="eas:i18n('Application Catalogue')"/>
	<xsl:variable name="catalogueIntro" select="eas:i18n('Please click on one of the Applications below to navigate to the required view')"/>
	<!-- END GENERIC CATALOGUE SETUP VARIABES -->

	<!-- START CATALOGUE SPECIFIC VARIABLES -->
	<xsl:variable name="appListByName" select="eas:get_report_by_name('Core: Application Provider Catalogue by Name')"/>
		...
		<html>
			...
			<body>
				...
				<xsl:call-template name="RenderCatalogueByCategoryJS">
					...
					<xsl:with-param name="orphanSubject">Application Family</xsl:with-param>
