...
<xsl:stylesheet xpath-default-namespace="http://protege.stanford.edu/xml" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:functx="http://www.functx.com" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<!-- 21.01.2012	JWC	Updated for Application Distribution Change Analysis -->
	...
	<!-- param1 = the instance ID of the application -->
	...
	<xsl:variable name="linkClasses" select="('Physical_Process', 'Application_Provider', 'Business_Process', 'Business_Activity')"/>
	...
	<xsl:variable name="roadmapConstantName" select="'Application_Roadmap'"/>

	<!-- CHANGE THIS Change Analysis LABEL TO THE NAME OF THE Change Analysis -->
	<xsl:variable name="gapReportLabel">
		<xsl:value-of select="eas:i18n('Application Change Analysis - Application Distribution')"/>
	</xsl:variable>

	<!-- CHANGE THIS TEXT TO LIST THE TYPES OF ELEMENTS IN SCOPE FOR THE Change Analysis (USED IN DESCRIPTIONS) -->
	<xsl:variable name="gapReportElementDesc" select="'Application Distributions (the use of the Application by Business Processes at a location)'"/>
	...
	<xsl:template name="PrintElementTypePrefix">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'APP_PRO_TO_PHYS_BUS_RELATION'">
				<!-- <span class="small normal">Application used by</span><br/>
				</xsl:when> -->
				<xsl:variable name="aPhysProcID" select="own_slot_value[slot_reference = 'apppro_to_physbus_to_busproc']/value"/>
				<xsl:variable name="aPhysProc" select="/node()/simple_instance[name = $aPhysProcID]"/>
				<xsl:choose>
					<xsl:when test="$aPhysProc/type = 'Physical_Process'">
						<xsl:variable name="aSiteIDList" select="$aPhysProc/own_slot_value[slot_reference = 'process_performed_at_sites']/value"/>
						<xsl:variable name="aSiteList" select="/node()/simple_instance[name = $aSiteIDList]"/>
						<xsl:for-each-group select="$aSiteList" group-by="name">
							<span class="small normal">Usage @ <xsl:value-of select="current-group()[1]/own_slot_value[slot_reference = 'name']/value"/></span>
							<br/>
						</xsl:for-each-group>
					</xsl:when>
					<xsl:otherwise>
						<!-- Physical Activity -->
						<xsl:variable name="aSiteID" select="$aPhysProc/own_slot_value[slot_reference = 'activity_performed_at_site']/value"/>
						<xsl:variable name="aSite" select="/node()/simple_instance[name = $aSiteID]"/>
						<span class="small normal">Usage @ <xsl:value-of select="$aSite/own_slot_value[slot_reference = 'name']/value"/></span>
						<br/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
	...
	<xsl:template name="PrintElementDisplayLabel">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'APP_PRO_TO_PHYS_BUS_RELATION'">
				<!-- Render the logical process name -->
				<xsl:variable name="aPhysProcID" select="own_slot_value[slot_reference = 'apppro_to_physbus_to_busproc']/value"/>
				<xsl:variable name="aPhysProc" select="/node()/simple_instance[name = $aPhysProcID]"/>
				<xsl:choose>
					<xsl:when test="$aPhysProc/type = 'Physical_Process'">
						<xsl:variable name="aLogProcID" select="$aPhysProc/own_slot_value[slot_reference = 'implements_business_process']/value"/>
					...
					<xsl:otherwise>
						<!-- Physical Activity -->
						<xsl:variable name="aLogActID" select="$aPhysProc/own_slot_value[slot_reference = 'instance_of_business_activity']/value"/>
						<xsl:variable name="aLogAct" select="/node()/simple_instance[name = $aLogActID]"/>
						<!--<xsl:value-of select="$aLogAct/own_slot_value[slot_reference='name']/value" />-->
						<xsl:call-template name="RenderInstanceLink">
							<xsl:with-param name="theSubjectInstance" select="$aLogAct"/>
							<xsl:with-param name="theXML" select="$reposXML"/>
							<xsl:with-param name="viewScopeTerms" select="$viewScopeTerms"/>
							<xsl:with-param name="anchorClass" select="'text-white'"/>
						</xsl:call-template>
	...
	<xsl:template match="knowledge_base">
		<xsl:choose>
			<xsl:when test="string-length($viewScopeTermIds) > 0">
				<xsl:variable name="roadmapConstant" select="/node()/simple_instance[own_slot_value[slot_reference = 'report_constant_short_name']/value = $roadmapConstantName]"/>
			...
			<xsl:otherwise>
				<xsl:variable name="roadmapConstant" select="/node()/simple_instance[(own_slot_value[slot_reference = 'report_constant_short_name']/value = $roadmapConstantName)][1]"/>
				<xsl:variable name="roadmap" select="/node()/simple_instance[name = $roadmapConstant/own_slot_value[slot_reference = 'report_constant_ea_elements']/value]"/>
				<xsl:call-template name="BuildPage">
					<xsl:with-param name="roadmap" select="$roadmap"/>
				</xsl:call-template>
	...
	<xsl:template name="BuildPage">
		...
		<!-- Get all the App Pro Roles of the selected Application -->
		<xsl:variable name="appProRoles" select="/node()/simple_instance[own_slot_value[slot_reference = 'role_for_application_provider']/value = $param1]"/>
		<!-- Find all the APP_PRO_TO_PHYS_BUS_RELATION for these roles -->
		<!-- All gap elements are all the APP_PRO_TO_PHYS_BUS_RELATION in which the roles are used -->
		<xsl:variable name="allGapElements" select="/node()/simple_instance[(type = 'APP_PRO_TO_PHYS_BUS_RELATION') and ((own_slot_value[slot_reference = 'apppro_to_physbus_from_appprorole']/value = $appProRoles/name))]"/>
		<xsl:variable name="strategicPlans" select="/node()/simple_instance[(type = 'Application_Strategic_Plan') and (name = $roadmapRelations/own_slot_value[slot_reference = ':roadmap_strategic_plans']/value)]"/>
		<xsl:variable name="currentStateElements" select="$allGapElements[name = $startArchState/own_slot_value[slot_reference = 'arch_state_application_relations']/value]"/>
		<xsl:variable name="targetStateElements" select="$allGapElements[name = $targetArchState/own_slot_value[slot_reference = 'arch_state_application_relations']/value]"/>
					...
					<div class="row">
						...
						<div class="col-xs-12">
							...
							<div class="content-section">
								<p>
									<xsl:value-of select="eas:i18n('The table below provides a comparative analysis between')"/>
									<strong>
										<xsl:value-of select="eas:i18n('Target and Baseline')"/>
										<xsl:value-of select="$gapReportElementDesc"/>
									</strong>
									<xsl:value-of select="eas:i18n('by indicating what needs to change across')"/>
	...
	<xsl:template name="Matrix">
		...
		<div class="xAxisLabelBar bg-black text-white">
			<xsl:value-of select="eas:i18n('Baseline')"/>
		</div>
		<div class="clear"/>
		<div class="yAxisLabelBar bg-black text-white">
			<xsl:value-of select="eas:i18n('Target')"/>
		</div>
		<div>
			<!--setup the main matrix-->
			<table class="tableStyleMatrix dataTable table-header-background" id="dt_gap">
				<thead>
					<tr>
						...
						<!--<th class="cellWidth-180">Alpha</th>
						<th class="cellWidth-180">Beta</th>
						<th class="cellWidth-180 cellRAG_Red">Gamma</th>
						<th class="cellWidth-180">Delta</th>
						<th class="cellWidth-180">Epsilon</th>
						<th class="cellWidth-180">Zeta</th>-->
					</tr>
				</thead>
				<tbody>
					...
					<!--<tr>
						<td class="bg-midgrey text-white vAlignMiddle"><strong>Alpha</strong></td>
						<td><div class="matrixEnumIcon EnumIcon_Plus backColour9 small text-white"><xsl:value-of select="eas:i18n('Enhance')"/></div></td>
						<td>&#160;</td>
						<td>&#160;</td>
						<td>&#160;</td>
						<td>&#160;</td>
					...
					<tr>
						<td class="bg-midgrey text-white vAlignMiddle"><strong>Beta</strong></td>
						<td>&#160;</td>
						<td><div class="matrixEnumIcon EnumIcon_Tick backColour9 small text-white"><xsl:value-of select="eas:i18n('Keep')"/></div></td>
						<td>&#160;</td>
						<td>&#160;</td>
						<td>&#160;</td>
						<td>&#160;</td>
					...
					<tr>
						<td class="bg-midgrey text-white vAlignMiddle"><strong>Gamma</strong></td>
						<td>&#160;</td>
						<td>&#160;</td>
						<td>&#160;</td>
						<td><div class="matrixEnumIcon EnumIcon_Tick backColour9 small text-white"><xsl:value-of select="eas:i18n('Keep')"/></div></td>
					...
					<tr>
						<td class="bg-midgrey text-white vAlignMiddle"><strong>Epsilon</strong></td>
						<td>&#160;</td>
						<td>&#160;</td>
						<td>&#160;</td>
						<td>&#160;</td>
					...
					<tr>
						<td class="bg-midgrey text-white vAlignMiddle"><strong>Zeta</strong></td>
						<td>&#160;</td>
						<td>&#160;</td>
						<td>&#160;</td>
						<td>&#160;</td>
					...
					<tr>
						<td class="cellRAG_Green text-white vAlignMiddle"><strong>Eta</strong></td>
						<td>&#160;</td>
						<td>&#160;</td>
						<td><div class="matrixEnumIcon EnumIcon_HorizArrows backColour9 small text-white"><xsl:value-of select="eas:i18n('Replace')"/></div></td>
						<td>&#160;</td>
						<td>&#160;</td>
						<td>&#160;</td>
					</tr>
					<tr>
						<td class="cellRAG_Green text-white vAlignMiddle"><strong>Theta</strong></td>
						<td>&#160;</td>
						<td>&#160;</td>
						<td><div class="matrixEnumIcon EnumIcon_HorizArrows backColour9 small text-white"><xsl:value-of select="eas:i18n('Replace')"/></div></td>
						<td>&#160;</td>
