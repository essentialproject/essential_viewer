...
var viewerDate=$('#date').text(); 
var publishDate=sessionStorage.getItem("viewerDate");   
...
//console.log('viewAPIData',viewAPIData)
			var promise_loadViewerAPIData = function(apiDataSetURL) {
				...
				let martArray=sessionStorage.getItem("Core API: BusCap to App Mart Apps");  
				if(apiDataSetURL==viewAPIDataMart &amp;&amp; martArray &amp;&amp; (viewerDate == publishDate)){
					return JSON.parse(martArray)
				}else{
...
$('document').ready(function () {
	showEditorSpinner('Fetching Data')
			...
			{
				sessionStorage.setItem('Core API: BusCap to App Mart Apps', JSON.stringify(responses[1])); 
				removeEditorSpinner();
			...
			if(viewerDate == publishDate){
				//no change
			}
			else{
				// update date for new publish
				sessionStorage.setItem('viewerDate',viewerDate)
			}
