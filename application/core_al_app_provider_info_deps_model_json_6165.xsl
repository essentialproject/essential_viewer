...
<script id="iface-list-template" type="text/x-handlebars-template">
	...
	{{else}}
		...
		<foreignObject class="apiBox" height="25">
			<xsl:attribute name="style">;white-space: nowrap; margin-right:3px;overflow: hidden; text-overflow: ellipsis; display: inline-block; vertical-align: bottom</xsl:attribute>		
			<xsl:attribute name="width">{{#getItemWidth 'Api1'}}{{/getItemWidth}}</xsl:attribute>	
    ...
    {{#if this.apiApp}}{{else}}
		...
		{{#each this.to}}
			...
			<foreignObject eas-type="toInterfaceTarget" style="background-color:rgb(255,255,255);border-radius:5px; padding:3px" width="20" class="lineClick"   >
				...
				<xsl:attribute name="eas-int-id">{{id}}</xsl:attribute>
				<xsl:attribute name="eas-dep-id">{{this.sendsTo.0.dependencyId}}</xsl:attribute> 
				<xsl:attribute name="y">{{#geticonAPILineRow this.num}}{{/geticonAPILineRow}}</xsl:attribute>
				<xsl:attribute name="height">20</xsl:attribute>
				<xhtml> 
					<div><i class="fa fa-info-circle fa-sm"></i> {{{essRenderInstanceMenuLink this}}} </div>
						<div class="node-body-icon">{{#testMe this}}{{/testMe}}
							<i class="fa fa-info-circle  fa-sm"></i>
						</div> 
				</xhtml>
			</foreignObject>	
...
dataFromAPI.dependencies.fromInterfaces.forEach((e)=>{
	...
	dataFromAPI.dependencies.toInterfaces.forEach((e)=>{
		...
		if(e.sendsTo[0].name){ 
			...
			dependencyMap.push({"depId":e.sendsTo[0].dependencyId,"debug":"tip", "source": e.sendsTo[0].name, "target": e.name,"infoReps": e.sendsTo[0].infoReps,"infoViews": e.sendsTo[0].infoViews});
			}
...
$(document).ready(function() {
	let ww=wwidth
		...
		Handlebars.registerHelper('testMe', function (arg1, options) {
