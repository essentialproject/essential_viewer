...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="allDOs" select="/node()/simple_instance[type = 'Data_Object']"></xsl:variable>
	...
	<xsl:template match="knowledge_base">
		...
		<html>
						...
						<div class="col-xs-12">
							<div class="page-header">
								<h1>
									...
									<span class="text-darkgrey">Data Object Recursion</span>
								</h1>
							</div>
						</div>
			...
			<script>
					var dataObject = [<xsl:apply-templates select="$allDOs" mode="dataObjects"><xsl:with-param name="depth" select="0"/></xsl:apply-templates>];
	...
	<xsl:template match="node()" mode="dataObjects">
		...
		<xsl:variable name="specialisation" select="$allDOs[name = current()/own_slot_value[slot_reference = 'data_object_specialisations']/value]"></xsl:variable>
		<xsl:variable name="generalisation" select="$allDOs[name = current()/own_slot_value[slot_reference = 'data_object_generalisations']/value]"></xsl:variable>
		...
		"general":"<xsl:value-of select="$generalisation/name"/>",
		"special":"<xsl:value-of select="$specialisation/name"/>",
		"level":"<xsl:value-of select="$depth"/>","children": [<xsl:if test="$depth &lt; 10"><xsl:apply-templates select="$specialisation" mode="dataObjects"><xsl:with-param name="depth" select="$depth +1"/></xsl:apply-templates></xsl:if>] }<xsl:if test="not(position() = last())"><xsl:text>,
		</xsl:text></xsl:if>
	</xsl:template>
