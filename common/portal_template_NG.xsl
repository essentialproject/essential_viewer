...
-->
	<xsl:param name="ngPortal"/> 
	<xsl:param name="ngPortal_Section"/>  
	<xsl:param name="ngPortal_Panel"/>  
	<xsl:param name="ngPortal_Panel_Section"/>  
	<xsl:param name="ngExternal_Reference_Link"/>  
	<xsl:param name="ngIndividual_Actor"/>  
	<xsl:param name="ngGroup_Actor"/>  
	<xsl:param name="ngReport"/>
	<xsl:param name="ngEditors"/>
	<xsl:param name="ngConfigEditors"/>
	<xsl:param name="ngSimpleEditors"/>
	<xsl:param name="ngReport_Constant"/>
	<xsl:param name="ngREPORT_TO_PORTAL_SECTION_RELATION"/>
	<eas:apiRequests>
		{"apiRequestSet": [
			{
				"variable": "ngReport_Constant",
				"query": "/instances/type/Report_Constant"
			},
            {
                "variable": "ngPortal",
                "query": "/instances/type/Portal"
			},
			{
                "variable": "ngPortal_Section",
                "query": "/instances/type/Portal_Section"
			},
			{
                "variable": "ngPortal_Panel",
                "query": "/instances/type/Portal_Panel"
			},
			{
                "variable": "ngPortal_Panel_Section",
                "query": "/instances/type/Portal_Panel_Section"
			},
			{
                "variable": "ngExternal_Reference_Link",
                "query": "/instances/type/External_Reference_Link"
			},
			{
                "variable": "ngIndividual_Actor",
                "query": "/instances/type/Individual_Actor"
			},
			{
                "variable": "ngGroup_Actor",
			...
			{
                "variable": "ngREPORT_TO_PORTAL_SECTION_RELATION",
                "query": "/instances/type/REPORT_TO_PORTAL_SECTION_RELATION"
			},
			{
                "variable": "ngEditors",
                "query": "/instances/type/Editor"
			} ,
			{
                "variable": "ngConfigEditors",
                "query": "/instances/type/Configured_Editor"
			},
			{
                "variable": "ngSimpleEditors",
                "query": "/instances/type/Simple_Editor"
			}
			]} 
	</eas:apiRequests>
	...
	<xsl:variable name="anyPlainReports" select="$ngReport//simple_instance"/>
<!--	<xsl:variable name="allReports2" select="$anyReports[own_slot_value[slot_reference = 'report_is_enabled']/value = 'true']"/>-->
	<xsl:variable name="allPortals2" select="$ngPortal//simple_instance"/>
	<xsl:variable name="allPortalSections" select="$ngPortal_Section//simple_instance"/>
	<xsl:variable name="allPortalPanels" select="$ngPortal_Panel//simple_instance"/>
	<xsl:variable name="allPortalPanelSections" select="$ngPortal_Panel_Section//simple_instance"/>
	<xsl:variable name="allExtRefs" select="$ngExternal_Reference_Link//simple_instance"/>
	<xsl:variable name="allIndActors" select="$ngIndividual_Actor//simple_instance"/>
	<xsl:variable name="allOrgActors" select="$ngGroup_Actor//simple_instance"/>
	<xsl:variable name="allActors" select="$allIndActors union $allOrgActors"/>
	<xsl:variable name="allReportConstants" select="$ngReport_Constant//simple_instance"/>
	<xsl:variable name="allNormalEditors" select="$ngEditors//simple_instance"/>
	<xsl:variable name="allConfigEditors" select="$ngConfigEditors//simple_instance"/>
	<xsl:variable name="allSimpleEditors" select="$ngSimpleEditors//simple_instance"/>
	
	<xsl:variable name="allEditors" select="$allNormalEditors union $allConfigEditors union $allSimpleEditors"/>
	
	<xsl:variable name="anyReports" select="$anyPlainReports union $allEditors"/>
	<xsl:key name="anyReports_key" match="$anyReports" use="own_slot_value[slot_reference = 'report_is_enabled']/value"/>
	<xsl:variable name="allReports2" select="key('anyReports_key','true')"/>
	<xsl:variable name="qualifyingReports" select="$anyReports[name=$allReports2/own_slot_value[slot_reference='report_qualifying_report']/value]"/>
	<xsl:variable name="rtpsr" select="$ngREPORT_TO_PORTAL_SECTION_RELATION//simple_instance"/>

	<!--
	<xsl:variable name="allReports2" select="$anyReports[own_slot_value[slot_reference = 'report_is_enabled']/value = 'true']"/>
	-->
	<xsl:key name="allPortals2_key" match="$allPortals2" use="name"/>
	<xsl:key name="allPortalSections_key" match="$allPortalSections" use="own_slot_value[slot_reference = 'parent_portal']/value"/>
	<xsl:key name="allPortalPanels_key" match="$allPortalPanels" use="own_slot_value[slot_reference = 'panel_for_portal']/value"/>
	<xsl:key name="allPortalPanelSections_key" match="$allPortalPanelSections" use="own_slot_value[slot_reference = 'portal_parent_panel']/value"/>
	<xsl:key name="allExtRefs_key" match="$allExtRefs" use="own_slot_value[slot_reference = 'referenced_ea_instance']/value"/>
	<xsl:key name="allReportToPortalSectionRelation_key" match="$rtpsr" use="own_slot_value[slot_reference = 'r2psr_portal_section']/value"/>
	<xsl:variable name="currentPortal" select="key('allPortals2_key',$param1)"/> 
	<xsl:variable name="portalPanels" select="key('allPortalPanels_key',$currentPortal/name)"/>
	<xsl:variable name="portalPanelSections" select="key('allPortalPanelSections_key',$portalPanels/name)"/>
...
-->
	...
	<!--	<xsl:variable name="allReportToPortalSectionRelation" select="/node()/simple_instance[type = 'REPORT_TO_PORTAL_SECTION_RELATION']"/>
		...
		<xsl:variable name="reportsForSection" select="$allReports2[name = $allR2PSRForSection/own_slot_value[slot_reference = 'r2psr_report']/value]"/>
	...
	<xsl:template mode="sectionReports" match="node()">
		...
		<xsl:variable name="currentReport" select="$allReports2[name = $aCurNode/own_slot_value[slot_reference = 'r2psr_report']/value]"/>
	...
	<xsl:template mode="portalPanels" match="node()">
		...
		<xsl:variable name="panelReports" select="$allReports2[name = current()/own_slot_value[slot_reference = 'reports_for_portal_panel']/value]"/>
	...
	<xsl:template mode="portalPanelSections" match="node()">
		<xsl:variable name="sectionReports" select="$allReports2[name = current()/own_slot_value[slot_reference = 'portal_panel_section_reports']/value]"/>
