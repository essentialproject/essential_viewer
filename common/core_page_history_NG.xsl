...
-->
	...
	<xsl:param name="currentSubjectXML"/>
	<eas:apiRequests>
		{
			"apiRequestSet": [
				<!-- Get single instance with ID matching the "theSubjectID" parameter-->
				{"variable": "currentSubjectXML", "query": "/instances/id/{theSubjectID}"}
			]
		}
	</eas:apiRequests>
	<xsl:variable name="currentSubject" select="$currentSubjectXML//simple_instance"/>	
	<xsl:key name="utilitiesAllReportsKey" match="$utilitiesAllReports" use="own_slot_value[slot_reference = 'report_xsl_filename']/value"/>
...
<!--
		...
		<xsl:variable name="currentReport" select="key('utilitiesAllReportsKey', $theCurrentXSL)"/>
