...
<xsl:stylesheet exclude-result-prefixes="pro xalan xs functx fn eas" 
	...
	<xsl:template mode="RenderMenuItemModalEntries" match="node()">
		...
		<xsl:if test="eas:isUserAuthZ($thisModal)">
			...
			,<xsl:value-of select="$menuItemShortName"/>: {disabled: function(){return (typeof essIdeas === 'undefined' || essIdeas === null || !essIdeas.ready);}, name: "<xsl:value-of select="$menuItemLabel"/>", icon: "<xsl:value-of select="$menuItemIconName"/>", callback: <xsl:value-of select="$menuItemFunctionName"/>}<xsl:if test="not(position() = last())">,</xsl:if>
		</xsl:if>
