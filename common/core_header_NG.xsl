...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" exclude-result-prefixes="pro xalan xs functx eas ess user" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview" xmlns:user="http://www.enterprise-architecture.org/essential/vieweruserdata">
	...
	<xsl:import href="../common/user-settings/core_user_view_settings_templates_NG.xsl"/>
	<xsl:include href="../common/core_header_interactions_NG.xsl"/>
	<xsl:include href="../common/core_utilities_NG.xsl"/>
	<xsl:include href="../common/core_page_history_NG.xsl"/>
	<xsl:include href="../common/core_feedback_NG.xsl"/>
	...
	<xsl:param name="headerHomeReportConstantXML"/>
	<xsl:param name="allPortalsXML"/>
	<xsl:param name="allReportsXML"/>
	<xsl:param name="allViewerStylesXML"/>
	<eas:apiRequests>
		
		{
			"apiRequestSet": [
				{
				"variable": "headerHomeReportConstantXML",
				"query": "/instances/type/Report_Constant/slots?name=Home Page"
				},
				{
				"variable": "allPortalsXML",
				"query": "/instances/type/Portal"
				},
				{
				"variable": "allReportsXML",
				"query": "/instances/type/Report"
				},
				{
				"variable": "allViewerStylesXML",
				"query": "/instances/type/Viewer_Styling"
				}
			]
		}
		
	</eas:apiRequests>
	<xsl:variable name="headerHomeReportConstant" select="$headerHomeReportConstantXML//simple_instance"/>
	<xsl:variable name="allPortals" select="$allPortalsXML//simple_instance"/>
	<xsl:variable name="allReports" select="$allReportsXML//simple_instance"/>
	<xsl:variable name="allViewerStyles" select="$allViewerStylesXML//simple_instance"/>
	
	<xsl:key name="allViewerStylesKey" match="$allViewerStyles" use="own_slot_value[slot_reference = 'style_is_active']/value"/>
	...
	<xsl:key name="allPortalsKeyName" match="$allPortals" use="name"/>
	<!--<xsl:variable name="portalContantTemplateXSL" select="$allPortals[name = $portalConstant]/own_slot_value[slot_reference = 'portal_xsl_filename']/value"/>-->
	<xsl:variable name="preportalContantTemplateXSL" select="key('allPortalsKeyName', $portalConstant)"/>
	<xsl:variable name="portalContantTemplateXSL" select="$preportalContantTemplateXSL/own_slot_value[slot_reference = 'portal_xsl_filename']/value"/>
	...
	<xsl:key name="allPortals_key" match="$allPortals" use="own_slot_value[slot_reference = 'name']/value"/>
	<xsl:key name="allEnablesPortals_key" match="$allPortals" use="own_slot_value[slot_reference = 'portal_is_enabled']/value"/>

	<xsl:key name="allReports_key" match="$allReports" use="own_slot_value[slot_reference = 'name']/value"/>
	...
	<xsl:key name="utilitiesAllEditorsKey" match="$utilitiesAllEditors" use="name"/>
	...
	<xsl:variable name="dynQueryInstance" select="key('utilitiesAllEditorsKey','SYS: DYNAMIC QUERY TOOL')"/>
					...
					<a class="navbar-brand" onclick="location.href='{$homeHref}';">
						<div class="tenant-logo">
							<img alt="tenant logo"><!--
								...
								<xsl:variable name="preactiveViewerStyle" select="key('allViewerStylesKey','true')"/>
								<xsl:variable name="activeViewerStyle" select="$preactiveViewerStyle[1]"/>
