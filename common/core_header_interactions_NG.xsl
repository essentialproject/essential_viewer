...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml"
  ...
  <xsl:include href="../common/core_approvals_NG.xsl"/>
  <xsl:include href="../common/core_ideas_NG.xsl"/>

  <xsl:variable name="hdrUserId" select="$userData//user:email"/>
  ...
  <xsl:param name="sysIdeationConstantXML"/>
  <xsl:param name="thisRepoVersionXML"/>
  <xsl:param name="viewSubjectXML"/>
  <xsl:param name="sysApprovalRoleXML"/>
  <xsl:param name="sysUserXML"/>
  <xsl:param name="hdrActors2RoleXML"/>
  <eas:apiRequests>
		
		{
			"apiRequestSet": [
				  {
				"variable": "sysIdeationConstantXML",
				"query": "/instances/type/Report_Constant/slots?name=Ideation Enabled"
				},
				{
				"variable": "thisRepoVersionXML",
				"query": "/instances/type/Meta_Model_Version"
				},
				{
				"variable": "viewSubjectXML",
				"query": "/instances/id/{param1}"
				},
				{
				"variable": "sysApprovalRoleXML",
				"query": "/instances/type/Individual_Business_Role/slots?name=SYS_CONTENT_APPROVER"
				},
				{
				"variable": "sysUserXML",
				"query": "/instances/type/Individual_Actor/slots?email={hdrUserId}"
				},
				{
				"variable": "hdrActors2RoleXML",
  ...
  <xsl:variable name="sysIdeationConstantList" select="$sysIdeationConstantXML//simple_instance"/>
	<xsl:variable name="sysIdeationConstant" select="$sysIdeationConstantList/own_slot_value[slot_reference = 'report_constant_value']/value"/>
  <!-- <xsl:variable name="sysIdeationConstant" select="''"/> -->

  <xsl:variable name="thisRepoVersionList" select="$thisRepoVersionXML//simple_instance"/>
<xsl:variable name="thisRepoVersion" select="$thisRepoVersionList[1]/own_slot_value[slot_reference = 'meta_model_version_id']/value"/>
  <!-- <xsl:variable name="thisRepoVersion" select="'6.19'"/> -->

  <xsl:variable name="viewSubject" select="$viewSubjectXML//simple_instance"/>
  <!-- <xsl:variable name="viewSubject" select="()"/> -->

  <xsl:variable name="sysApprovalRole" select="$sysApprovalRoleXML//simple_instance"/>
  <!-- <xsl:variable name="sysApprovalRole" select="()"/> -->

  <xsl:variable name="sysUser" select="$sysUserXML//simple_instance"/>
  <!-- <xsl:variable name="sysUser" select="()"/> -->

  <xsl:variable name="hdrActors2Roles" select="$hdrActors2RoleXML//simple_instance"/>
  <!-- <xsl:variable name="hdrActors2Roles" select="()"/> -->

  <xsl:variable name="sysIdeationIsOn" select="string-length($sysIdeationConstant)"/>

  <!-- <xsl:variable name="sysUserAsApprover" select="$hdrActors2Roles[(own_slot_value[slot_reference = 'act_to_role_from_actor']/value = $sysUser/name) and (own_slot_value[slot_reference = 'act_to_role_to_role']/value = $sysApprovalRole/name)]"/> -->
  <xsl:variable name="sysUserAsApprover" select="()"/>
  ...
  <xsl:template mode="RenderViewerDataSetAPIDetails" match="node()">
    ...
    <xsl:variable name="userIdParam">&amp;essuser=<xsl:value-of select="$thisUserId"/>
    </xsl:variable>
