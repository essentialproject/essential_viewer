				...
				<table>
					...
					<tbody>
						<xsl:for-each select="$anExternalRefList">
							...
							<xsl:variable name="anExternalRef" select="/node()/simple_instance[name = $anInstName]"/>
							...
							<xsl:variable name="anExternalReposName" select="/node()/simple_instance[name = $anExternalReposInst]/own_slot_value[slot_reference = 'name']/value"/>
							<tr>
								<td>
