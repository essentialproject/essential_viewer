...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="targetReportClasses" select="eas:getClassNamesForReport($targetReport)"/>
	<!-- END GENERIC CATALOGUE SETUP VARIABES -->
	...
	<xsl:variable name="businessProcess2Family" select="/node()/simple_instance[own_slot_value[slot_reference = 'busprocttype_to_busprocfam_busproctype']/value = $businessProcess/name]"/>
	<xsl:variable name="businessProcessFamily" select="/node()/simple_instance[name = $businessProcess2Family/own_slot_value[slot_reference = 'busprocttype_to_busprocfam_busprocfam']/value]"/>
					...
					<div class="row">
						<div class="col-xs-12">
							<div class="page-header">
								...
								<xsl:if test="not(count(concat($targetReportId, $targetMenuShortName)) > 0)">
	...
	<xsl:template match="node()" mode="processRow">
		...
		<xsl:variable name="thisProcess2Family" select="$businessProcess2Family[own_slot_value[slot_reference = 'busprocttype_to_busprocfam_busproctype']/value = $this/name]"/>
		<xsl:variable name="thisfamily" select="$businessProcessFamily[name = $thisProcess2Family/own_slot_value[slot_reference = 'busprocttype_to_busprocfam_busprocfam']/value]"/>

		<tr>
			<td>
				<xsl:choose>
					<xsl:when test="current()/type = $targetReportClasses">
            ...
            <td>
            	<xsl:choose>
            		<xsl:when test="$thisfamily/type = $targetReportClasses">
            			<xsl:call-template name="RenderInstanceLink">
