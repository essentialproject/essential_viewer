...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
    ...
    <xsl:key name="allBusProcesses" match="/node()/simple_instance[type='Business_Process']" use="own_slot_value[slot_reference = 'implemented_by_physical_business_processes']/value"/>
    <xsl:variable name="allBusProcesses" select="key('allBusProcesses', $allPhysProcessesCount/name)"/>
    ...
    <xsl:variable name="allProcessSites" select="key('allSites', $allPhysProcessesCount/own_slot_value[slot_reference = 'process_performed_at_sites']/value)"/>

    <xsl:key name="sitesKey" match="$allSites" use="own_slot_value[slot_reference = 'site_geographic_location']/value"/>
    <xsl:key name="costCompKey" match="/node()/simple_instance[supertype='Cost_Component']" use="name"/>
	...
	<xsl:variable name="inScopeCostComponents" select="key('costCompKey', $inScopeCosts/own_slot_value[slot_reference = 'cost_components']/value)"/>
	<xsl:variable name="inScopeCostTypes" select="/node()/simple_instance[type='Cost_Component_Type'][name = $inScopeCostComponents/own_slot_value[slot_reference = 'cc_cost_component_type']/value]"/>
  ...
  <xsl:key name="allGeosRegions" match="/node()/simple_instance[type='Geographic_Region']" use="name"/> 
    ...
    <xsl:variable name="allInScopeGeosRegions" select="key('allGeosRegions', $allCtrySites/own_slot_value[slot_reference = 'site_geographic_location']/value)"/>
...
<!-- order here has to map the markers order -->     
  ...
  values={<xsl:apply-templates select="$allInScopeGeosRegions" mode="getCountriesCount"/>}
  let locs = [ <xsl:apply-templates select="$allSites" mode="getMarkers"/> ];
       ...
       labels: {
        markers: {
          render: function(index){
            return locs[index].name;
          }
      }},
      onMarkerClick: function(event, index) {
                      // alter the weburl
                      location.href=locs[index].weburl;
                  },
      
      onMarkerTipShow: function(event, label, index){
        label.html(
          '<b># Processes:'+locs[index].processes+'</b><br/>'
        );
        },
       series: {
          regions: [{
 ...
 <xsl:template match="node()" mode="getMarkers">
     ...
     <xsl:if test="$lat"> {latLng: [<xsl:value-of select="$lat"/>,<xsl:value-of select="$long"/>], name: '<xsl:value-of select="$thisgeoLocation/own_slot_value[slot_reference='name']/value"/>', style: {fill: '#faa053'}, id:'<xsl:value-of select="$thisgeoLocation/name"/>', "processes":<xsl:value-of select="count($processAtSite)"/>, "weburl": "<xsl:call-template name="RenderLinkText">
  ...
  <xsl:template match="node()" mode="getCountriesCount"> 
 
          <xsl:variable name="allGeosInRegion" select="$geoLocation[name=current()/own_slot_value[slot_reference = 'gr_locations']/value]"/> 
          <xsl:variable name="allthisSites" select="key('sitesKey', $allGeosInRegion/name) union key('sitesKey',current()/name)"/> 
          <xsl:variable name="allthisProcessSites" select="key('allPhysProcessesKey', $allthisSites/name)"/>
           "<xsl:value-of select="current()/own_slot_value[slot_reference='gr_region_identifier']/value"/>":<xsl:value-of select="count($allthisProcessSites/name)"/><xsl:if test="position()!=last()">,</xsl:if>
  </xsl:template> 
