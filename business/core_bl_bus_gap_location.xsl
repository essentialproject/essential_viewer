...
<xsl:stylesheet xpath-default-namespace="http://protege.stanford.edu/xml" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:functx="http://www.functx.com" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="linkClasses" select="('Site')"/>
	...
	<xsl:variable name="gapReportLabel">
		<xsl:value-of select="eas:i18n('Location Change Analysis')"/>
	</xsl:variable>

	<!-- CHANGE THIS TEXT TO LIST THE TYPES OF ELEMENTS IN SCOPE FOR THE Change Analysis (USED IN DESCRIPTIONS) -->
	<xsl:variable name="gapReportElementDesc" select="'locations of Offices, Installations and Outlets etc.'"/>
	...
	<xsl:template name="PrintElementTypePrefix">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Site'">
	...
	<xsl:template name="BuildPage">
		...
		<xsl:variable name="phsProcessSites" select="$allSites[name = $allPhysProcess/own_slot_value[slot_reference = 'process_performed_at_sites']/value]"/>
		<xsl:variable name="allActors" select="/node()/simple_instance[(type = 'Individual_Actor') or (type = 'Group_Actor')]"/>
		<xsl:variable name="allActorSites" select="$allSites[name = $allActors/own_slot_value[slot_reference = 'actor_based_at_site']/value]"/>

		<xsl:variable name="allGapElements" select="($phsProcessSites union $allActorSites)"/>
		...
		<xsl:variable name="currentStateElements" select="$allGapElements[name = $startArchState/own_slot_value[(slot_reference = 'arch_state_business_physical')]/value]"/>
		<xsl:variable name="targetStateElements" select="$allGapElements[name = $targetArchState/own_slot_value[(slot_reference = 'arch_state_business_physical')]/value]"/>
