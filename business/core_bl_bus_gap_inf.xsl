...
<xsl:stylesheet xpath-default-namespace="http://protege.stanford.edu/xml" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:functx="http://www.functx.com" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="linkClasses" select="('Information_View')"/>
	...
	<xsl:variable name="gapReportLabel">
		<xsl:value-of select="eas:i18n('Information Change Analysis')"/>
	</xsl:variable>

	<!-- CHANGE THIS TEXT TO LIST THE TYPES OF ELEMENTS IN SCOPE FOR THE Change Analysis (USED IN DESCRIPTIONS) -->
	<xsl:variable name="gapReportElementDesc" select="'Business Information'"/>
	...
	<xsl:template name="PrintElementTypePrefix">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Information_View'">
	...
	<xsl:template name="BuildPage">
		...
		<xsl:variable name="allBUSPROCTYPE_TO_INFOVIEW_RELATION" select="/node()/simple_instance[type = 'BUSPROCTYPE_TO_INFOVIEW_RELATION']"/>
		<!--<xsl:variable name="allBusProInfoRelationInfoViews" select="$allBUSPROCTYPE_TO_INFOVIEW_RELATION/own_slot_value[slot_reference='busproctype_to_infoview_to_infoview']/value"></xsl:variable>-->
		...
		<xsl:variable name="allGapElements" select="$allInfoViews[name = $allBUSPROCTYPE_TO_INFOVIEW_RELATION/own_slot_value[slot_reference = 'busproctype_to_infoview_to_infoview']/value]"/>
