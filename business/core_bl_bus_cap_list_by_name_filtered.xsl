                ...
                {{#each this.caps}} 
                        <div class="col-xs-3">
                            <div class="caps bottom-5">
								{{#essRenderInstanceLinkSelect this 'Business_Capability'}}{{/essRenderInstanceLinkSelect}}<xsl:text> </xsl:text> {{this.name}}
                            </div>
                        </div>  
                 {{/each}}    
			</script>
				...
				if (targetReport.length &gt; 1) {
					...
					if (instance != null) {
						...
						if (linkMenuName != null) {
							...
							instanceLink = '<button class="ebfw-confirm-instance-selection btn btn-default btn-xs right-15" onclick="location.href=&quot;' + linkHref + '&quot;" id="' + linkId+'"><i class="text-success fa fa-check-circle right-5"></i><xsl:value-of select="eas:i18n('Select')"/></button>'
