                ...
                {{#each this.caps}} 
                        <div class="col-xs-4">
                            <div class="caps bottom-5">
                                <i class="fa fa-caret-right"> </i> {{#if ../this.type}} {{#essRenderInstanceLink this}}{{/essRenderInstanceLink}}{{else}}{{{this.link}}}{{/if}} 
                            </div>
                        </div>  
                 {{/each}}    
			</script>
	...
	<xsl:template name="RenderViewerAPIJSFunction">
		...
		$('document').ready(function () {
			...
			function essGetMenuName(instance) {
				...
				if ((instance != null) &amp;&amp;
					(instance.meta != null) &amp;&amp;
					(instance.meta.anchorClass != null)) {
					menuName = esslinkMenuNames['busCapGenMenu'];
				} else if (instance.class != null) {
					menuName = esslinkMenuNames['Business_Capability'];
				}
				return menuName;
			}
