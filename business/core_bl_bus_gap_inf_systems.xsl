...
<xsl:stylesheet xpath-default-namespace="http://protege.stanford.edu/xml" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:functx="http://www.functx.com" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="gapReportLabel">
		<xsl:value-of select="eas:i18n('Information System Change Analysis')"/>
	</xsl:variable>

	<!-- CHANGE THIS TEXT TO LIST THE TYPES OF ELEMENTS IN SCOPE FOR THE Change Analysis (USED IN DESCRIPTIONS) -->
	<xsl:variable name="gapReportElementDesc" select="'Information Systems supporting Business Processes'"/>
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Application_Provider'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Information System')"/>
				</span>
				<br/>
			</xsl:when>
		</xsl:choose>
	...
	<xsl:template name="BuildPage">
		...
		<!--Get all APP_PRO_TO_PHY_BUS_REALTIONS-->
		<xsl:variable name="allAPP_PRO_TO_PHYS_BUS_RELATION" select="/node()/simple_instance[type = 'APP_PRO_TO_PHYS_BUS_RELATION']"/>
		<!--Get the slot for the Application Provider Role-->
		<xsl:variable name="relevantAppRoles" select="/node()/simple_instance[name = $allAPP_PRO_TO_PHYS_BUS_RELATION/own_slot_value[slot_reference = 'apppro_to_physbus_from_appprorole']/value]"/>
		<!--Get all Application Provider Roles who are associated with a APP_PRO_TO_PHYS_BUS_RELATION-->
		<!--<xsl:variable name="relevantApp" select="/node()/simple_instance[name=$allAPP_PRO_TO_PHYS_BUS_RELATION_apps/own_slot_value[slot_reference='role_for_application_provider']/value]"/>
		-->
		<xsl:variable name="allGapElements" select="/node()/simple_instance[name = $relevantAppRoles/own_slot_value[slot_reference = 'role_for_application_provider']/value]"/>
