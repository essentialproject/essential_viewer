...
<xsl:stylesheet xpath-default-namespace="http://protege.stanford.edu/xml" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:functx="http://www.functx.com" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="linkClasses" select="('Business_Process', 'Business_Activity', 'Business_Task', 'Business_Rule')"/>
	...
	<xsl:variable name="gapReportLabel">
		<xsl:value-of select="eas:i18n('Business Process Change Analysis')"/>
	</xsl:variable>

	<!-- CHANGE THIS TEXT TO LIST THE TYPES OF ELEMENTS IN SCOPE FOR THE Change Analysis (USED IN DESCRIPTIONS) -->
	<xsl:variable name="gapReportElementDesc" select="'Businesss Processes, Activities, Tasks and Rules'"/>
	...
	<xsl:template name="PrintElementTypePrefix">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Business_Process'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Business Process')"/>
				</span>
				<br/>
			</xsl:when>
			<xsl:when test="$elementType = 'Business_Activity'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Business Activity')"/>
				</span>
				<br/>
			</xsl:when>
			<xsl:when test="$elementType = 'Business_Task'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Business Task')"/>
				</span>
				<br/>
			</xsl:when>
			<xsl:when test="$elementType = 'Business_Rule'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Business Rule')"/>
				</span>
				<br/>
			</xsl:when>
		</xsl:choose>
	...
	<xsl:template name="BuildPage">
		...
		<xsl:variable name="allGapElements" select="/node()/simple_instance[(type = 'Business_Process') or (type = 'Business_Activity') or (type = 'Business_Task') or (type = 'Business_Rule')]"/>
		...
		<xsl:variable name="currentStateElements" select="$allGapElements[name = $startArchState/own_slot_value[slot_reference = 'arch_state_business_logical']/value]"/>
		<xsl:variable name="targetStateElements" select="$allGapElements[name = $targetArchState/own_slot_value[slot_reference = 'arch_state_business_logical']/value]"/>
					...
					<div class="row">
						...
						<div class="col-xs-12">
							...
							<div class="content-section">
								<p><xsl:value-of select="eas:i18n('The table below provides a comparative analysis between')"/>&#160;<strong><xsl:value-of select="eas:i18n('Target and Baseline')"/>&#160;<xsl:value-of select="$gapReportElementDesc"/></strong>&#160;<xsl:value-of select="eas:i18n('by indicating what needs to change across')"/>&#160;<xsl:value-of select="$orgName"/></p>
								<div>
									<script>
	...
	<xsl:template name="Matrix">
		...
		<div class="matrixContainer no-padding col-xs-11">
			...
			<table class="tableStyleMatrix table-borders table-header-background" id="dt_gap">
				<thead>
					<tr>
						...
						<xsl:for-each select="$currentStateElements">
							...
							<xsl:choose>
								<xsl:when test="count($plansForElement[own_slot_value[slot_reference = 'strategic_planning_action']/value = $removePlanningAction/name]) > 0">
									<th class="cellRAG_Red cellWidth-180" style="background-color: #ba2401;">
