...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="linkClasses" select="('Business_Objective')"/>
		...
		<xsl:choose>
			<xsl:when test="string-length($viewScopeTermIds) > 0">
				...
				<xsl:variable name="pageLabel">
					<xsl:value-of select="eas:i18n('Business Objective Catalogue by Name')"/>
				</xsl:variable>
				<xsl:call-template name="BuildPage">
					<xsl:with-param name="pageLabel" select="$pageLabel"/>
					<xsl:with-param name="inScopeObjectives" select="$allBusObjectives[own_slot_value[slot_reference = 'element_classified_by']/value = $viewScopeTerms/name]"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
				<xsl:call-template name="BuildPage">
					<xsl:with-param name="inScopeObjectives" select="$allBusObjectives"/>
				</xsl:call-template>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="BuildPage">
		...
		<xsl:param name="inScopeObjectives"/>
					...
					<div class="row">
						...
						<div id="sectionApplicationServiceCatalogue">
							<div class="col-xs-12">
								...
								<p><xsl:value-of select="eas:i18n('Please click on one of the Business Objectives below to navigate to the required view')"/>.</p>
								...
								<div class="AlphabetQuickJumpLinks hidden-xs">
									...
									<xsl:variable name="anInFocusInstances" select="$inScopeObjectives"></xsl:variable>
								...
								<xsl:call-template name="Index">
									...
									<xsl:with-param name="inScopeInstances" select="$inScopeObjectives"/>
								</xsl:call-template>
	...
	<xsl:template name="Index">
		...
		<!-- Business Objectives START HERE -->
		<div class="catalogItems">
			<ul>
				<xsl:apply-templates select="$inScopeInstances[((starts-with(own_slot_value[slot_reference = 'name']/value, $letter)) or (starts-with(own_slot_value[slot_reference = 'name']/value, $letterLow)))]" mode="BusinessObjective">
					<xsl:sort order="ascending" select="own_slot_value[slot_reference = 'name']/value"/>
				</xsl:apply-templates>
			</ul>
			<!-- Business Objectives END HERE -->
		</div>
