...
<xsl:stylesheet version="2.0" xpath-default-namespace="http://protege.stanford.edu/xml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="linkClasses" select="('Composite_Product_Type', 'Product_Type')"/>
	<!-- END GENERIC CATALOGUE SETUP VARIABES -->

	<!-- Get all of the Application Services in the repository -->
	<xsl:variable name="allBusServices" select="/node()/simple_instance[type = ('Composite_Product_Type', 'Product_Type')]"/>
		...
		<xsl:choose>
			<xsl:when test="string-length($viewScopeTermIds) > 0">
				...
				<xsl:variable name="pageLabel">
					<xsl:value-of select="eas:i18n('Business Service Catalogue by Name')"/>
				</xsl:variable>
				<xsl:call-template name="BuildPage">
					<xsl:with-param name="pageLabel" select="$pageLabel"/>
					<xsl:with-param name="inScopeBusSvcs" select="$allBusServices[own_slot_value[slot_reference = 'element_classified_by']/value = $viewScopeTerms/name]"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
				<xsl:call-template name="BuildPage">
					<xsl:with-param name="inScopeBusSvcs" select="$allBusServices"/>
				</xsl:call-template>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
				...
				<div class="container-fluid">
					...
					<div class="row">
						<div class="col-xs-12">
							...
							<p><xsl:value-of select="eas:i18n('Please click on one of the Business Services below to view its detailed specification')"/>.</p>
							...
							<div class="AlphabetQuickJumpLinks hidden-xs">
								...
								<xsl:variable name="anInFocusInstances" select="$inScopeBusSvcs"></xsl:variable>
							...
							<xsl:call-template name="Index">
								...
								<xsl:with-param name="busServices" select="$inScopeBusSvcs"/>
							</xsl:call-template>
	...
	<xsl:template name="Index">
		...
		<div class="catalogItems">
			<ul>
				<xsl:apply-templates select="$busServices[((starts-with(own_slot_value[slot_reference = 'name']/value, $letter)) or (starts-with(own_slot_value[slot_reference = 'name']/value, $letterLow)))]" mode="BusinessService">
	...
	<xsl:template name="eas:renderIndex">
		...
		<xsl:for-each select="$anIndexKeys">
			<xsl:call-template name="Index">
				...
				<xsl:with-param name="busServices" select="$theInFocusInstances"/>
			</xsl:call-template>
