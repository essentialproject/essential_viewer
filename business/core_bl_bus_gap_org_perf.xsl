...
<xsl:stylesheet xpath-default-namespace="http://protege.stanford.edu/xml" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xslt" xmlns:pro="http://protege.stanford.edu/xml" xmlns:eas="http://www.enterprise-architecture.org/essential" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:functx="http://www.functx.com" xmlns:ess="http://www.enterprise-architecture.org/essential/errorview">
	...
	<xsl:variable name="linkClasses" select="('Business_Objective', 'Business_Driver')"/>
	...
	<xsl:variable name="gapReportLabel">
		<xsl:value-of select="eas:i18n('Business Performance Change Analysis')"/>
	</xsl:variable>

	<!-- CHANGE THIS TEXT TO LIST THE TYPES OF ELEMENTS IN SCOPE FOR THE Change Analysis (USED IN DESCRIPTIONS) -->
	<xsl:variable name="gapReportElementDesc" select="'Drivers, Objectives or Measures'"/>
	...
	<xsl:template name="PrintElementTypePrefix">
		...
		<xsl:choose>
			<xsl:when test="$elementType = 'Business_Objective'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Business Objective')"/>
				</span>
				<br/>
			</xsl:when>
			<xsl:when test="$elementType = 'Business_Driver'">
				<span class="small normal">
					<xsl:value-of select="eas:i18n('Business Driver')"/>
				</span>
				<br/>
			</xsl:when>
		</xsl:choose>
	...
	<xsl:template name="BuildPage">
		...
		<xsl:variable name="allGapElements" select="/node()/simple_instance[(type = 'Business_Driver') or (type = 'Business_Objective')]"/>
		...
		<xsl:variable name="currentStateElements" select="$allGapElements[name = $startArchState/own_slot_value[slot_reference = 'arch_state_business_conceptual']/value]"/>
		<xsl:variable name="targetStateElements" select="$allGapElements[name = $targetArchState/own_slot_value[slot_reference = 'arch_state_business_conceptual']/value]"/>
