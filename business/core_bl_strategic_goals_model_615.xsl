					...
					<div class="row">
						...
						<div class="col-xs-12">
							...
							<div class="content-section">
								...
								<div class="simple-scroller" id="goalsTable"> 
...
<script id="table-template" type="text/x-handlebars-template">
	<table class="table table-bordered table-header-background">
			<thead>
				<tr>
...
$(document).ready(function() {
	...
	$('#goalsTable').html(tableTemplate(drivers));
});
</script>
			</body>
		</html>
...
<xsl:template match="node()" mode="drivers">
		...
		{"debug":"<xsl:value-of select="$thisGoals/name"/>",
		"debug2":"<xsl:value-of select="$this/own_slot_value[slot_reference='bd_motivated_objectives']/value"/>",
...
<xsl:template match="node()" mode="goals">
	<xsl:variable name="this" select="current()"/>
	<xsl:variable name="thisObjs" select="$allBusObjectives[name=$this/own_slot_value[slot_reference='objective_supported_by_objective']/value]"/>
	{"goal":"<xsl:value-of select="$this/own_slot_value[slot_reference='name']/value"/>", "objectives":[<xsl:apply-templates select="$thisObjs" mode="objs"/>]}<xsl:if test="position()!=last()">,</xsl:if> 
</xsl:template>   
...
<xsl:template match="node()" mode="objs">
	...
	<xsl:variable name="busObjServtoObj" select="$allBusinessServiceOBJ[name = current()/own_slot_value[slot_reference = 'bo_performance_measures']/value]"/>		
		...
		<xsl:variable name="busObjIndividualOwners" select="$allIndividualActors[name = current()/own_slot_value[slot_reference = 'bo_owners']/value]"/>
		<xsl:variable name="busObjTargetDate" select="/node()/simple_instance[name = current()/own_slot_value[slot_reference = 'bo_target_date']/value]"/>
	{"objective":"<xsl:call-template name="RenderMultiLangInstanceName">
            <xsl:with-param name="theSubjectInstance" select="current()"/>
