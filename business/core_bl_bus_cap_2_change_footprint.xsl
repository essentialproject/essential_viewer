...
//						$(".threeColModel_NumberHeatmap").vAlign();					
                ...
                <script type="text/javascript">
                    var objectives=[<xsl:apply-templates select="$busObjectives" mode="objectives"></xsl:apply-templates>];
...
<xsl:template match="node()" mode="PrintChildFrontCap">
	...
	<xsl:template match="node()" mode="PrintGrandChildFrontCap">
		...
		<div class="threeColModel_ObjectContainer">
            <xsl:call-template name="cornerHightlight">
    ...
    <xsl:template name="cornerHightlight">
		...
		<xsl:variable name="triangleColour">
			<xsl:choose>
				<xsl:when test="$activityTotal= 0">cornerTriangleGrey</xsl:when>
				<xsl:when test="($activityTotal &gt; 0) and ($activityTotal &lt; 3)">cornerTriangleLightGreen</xsl:when>
				<xsl:when test="$activityTotal &gt; 2">cornerTriangleBlue</xsl:when>
			</xsl:choose>	
		</xsl:variable>
	
		<div style="z-index:1">
			<xsl:attribute name="class" select="concat('capCornerHighlight ', $triangleColour)"/>
		</div>
	</xsl:template>
