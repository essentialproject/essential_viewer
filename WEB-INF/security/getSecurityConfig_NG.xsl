...
<xsl:stylesheet 
    ...
    <!-- 26.01.2024 JP  Converted to be NG Viewer compatible -->
    
    <xsl:output method="xml" indent="yes"/>

    <xsl:param name="classificationGroupsXML"/>
    <xsl:param name="classificationsXML"/>
    <xsl:param name="defaultClassificationsXML"/>
    <xsl:param name="viewDefinitionsXML"/>
    ...
    <eas:apiRequests>
		
		{
			"apiRequestSet": [
				{
				"variable": "classificationGroupsXML",
				"query": "/instances/type/EA_Content_Classification_Group"
				},
                {
				"variable": "classificationsXML",
				"query": "/instances/type/EA_Content_Classification"
				},
                {
				"variable": "defaultClassificationsXML",
				"query": "/classes?className=EA_Default_Classification"
				},
                {
				"variable": "viewDefinitionsXML",
				"query": "/instances/supertype/Report/slots"
				}
			]
		}
		
	</eas:apiRequests>
	<xsl:variable name="classificationGroups" select="$classificationGroupsXML//simple_instance"/>
    <xsl:variable name="classifications" select="$classificationsXML//simple_instance"/>
    <xsl:variable name="defaultClassifications" select="$defaultClassificationsXML//class"/>
    <xsl:variable name="viewDefinitions" select="$viewDefinitionsXML//simple_instance"/>
