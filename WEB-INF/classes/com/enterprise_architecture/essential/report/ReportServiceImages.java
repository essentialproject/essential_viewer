/**
 * Copyright 2009-2019 Enterprise Architecture Solutions Ltd.
...
public class ReportServiceImages extends HttpServlet
{
	 ...
	 */
	private static final Logger itsLog = LoggerFactory.getLogger(ReportServiceImages.class);
	...
	{		
		...
		synchronized(theRequest)
		{
			...
			{
				itsLog.error("Exception encountered while parsing request to receive uploaded image: {}", aFileUploadEx);				
				theResponse.setStatus(ERROR_STATUS);
			}
			catch(Exception aWritingEx)
			{
				itsLog.error("Exception encountered while writing received image file: {}", aWritingEx);
				theResponse.setStatus(ERROR_STATUS);
			}
		}
		return;
