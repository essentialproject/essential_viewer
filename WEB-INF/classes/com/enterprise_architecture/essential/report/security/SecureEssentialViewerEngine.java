 ...
 * 14.01.2020	JWC Updated for Saxon 9.9.1
 * 25.02.2020	JWC	Rolled back to Saxon 9.2.1.5
...
public class SecureEssentialViewerEngine extends EssentialViewerEngine 
{
	 ...
	 */
	private static final Logger itsLog = LoggerFactory.getLogger(SecureEssentialViewerEngine.class);	
	...
	protected static final String VIEWER_SECURITY_MANAGER_SINGLETON_VARNAME = "viewerSecurityManager";
	...
	/**
	 * Clean up any resources when the engine is shut down/destroyed
	 * 
	 */
	public void closeResources()
	{
		if(itsSecurityMgr != null)
		{
			itsSecurityMgr.closeResources();
		}
	}
		 						...
		 						StringWriter theResultString) throws ServletException, IOException
	{
		...
		itsLog.debug("SecureViewer: authenticate attempt via session, got account object.");
		...
		if(anAccount == null)
		{
			...
			itsLog.debug("SecureViewer: Null account, do login process");
			
			// Send the request URL as a parameter so we know where to come back to?
			// Render a login View template.
			
 			// Just to be sure we invalidate the session and remove any OAuth token cookies.
										 ...
										 ViewerSecurityManager theSecurityMgr)
	{
		...
		String aUserData = theSecurityMgr.getUserData(theRequest, theResponse);
		...
		{
			// DEBUG TRACE
			itsLog.debug("TRACE: View: {}", aTemplateFile);
			itsLog.debug("TRACE: UserData XML: \n{}", aUserData);
			itsLog.debug("TRACE: View Classifications: \n{}" + aViewClassification);
			...
			itsLog.debug("TRACE = queryUserAuthZ(): {}", aUserAuthZResult);
			...
			String aRequestedInstanceID = ScriptXSSFilter.filter(theRequest.getParameter(REQUESTED_INSTANCE_ID_PARAM));
			...
			{				
				...
				// DEBUG TRACE
				itsLog.debug("SecureViewer.isUserAuthZForView(): TRACE == queryUserAuthZInstance(XSL): {}", aUserAuthZResult);				
			}
		}
		catch (Exception ex)
		{
											...
											String theContextPath)
	{
		...
		try
		{
			...
			TreeInfo anXMLSource = getXMLSourceDocument(tFactory, theSourceDocumentFullPath, aFactoryErrorListener);
									   ...
									   StringWriter theResultString)
	{
		...
		try
		{
			...
			// Use the cached (or not) reportXML			
			TreeInfo anXMLSource = getXMLSourceDocument(tFactory, aSourceDocumentFullPath, aFactoryErrorListener);
	...
	{
		ViewerSecurityManager aSecurityManager = (ViewerSecurityManager) itsServletContext.getAttribute(VIEWER_SECURITY_MANAGER_SINGLETON_VARNAME);
		if(null == aSecurityManager)
		{
			...
			itsServletContext.setAttribute(VIEWER_SECURITY_MANAGER_SINGLETON_VARNAME, aSecurityManager);
		}
