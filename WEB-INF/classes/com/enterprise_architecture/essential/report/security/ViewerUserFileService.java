 ...
 * 13.01.2021	JWC Change to use Apache Zip processing to handle incorrect UTF-8 encoding of Zip filenames
 */
...
public class ViewerUserFileService extends HttpServlet 
{
	 ...
	 */
	private static final Logger itsLog = LoggerFactory.getLogger(ViewerUserFileService.class);
	...
	{		
		...
		String aBearerToken = request.getParameter(ViewerSecurityManager.USER_ACCOUNT_URL);
		...
		String anAccount = aSecurityMgr.authenticateUserByToken(request, aBearerToken);
			...
			if(isAccountAuthorized(aSecurityMgr, request, anAccount))
			{
				...
				{		
					...
					{						
						...
						aRedirectURL = UriComponentsBuilder.fromUriString(aRedirectURL).encode(StandardCharsets.UTF_8).toUriString();
						itsLog.debug("The redirect URL is: {}", aRedirectURL);
	...
	protected void doPost(HttpServletRequest theRequest, HttpServletResponse theResponse) throws ServletException, IOException 
	{
		...
		itsLog.debug("Content Type: {}", aContentType);
		...
		if(anAccount != null)
		{
			if(isAccountAuthorized(aSecurityMgr, theRequest, anAccount))
			{
				...
				if(aContentType.contains(MULTIPART_FORM_CONTENT_TYPE))
				{
							...
							aFileContentType.contains(ALT_ZIP_CONTENT_TYPE))
					{
						if(anUnPack.equals("true"))
						{
							...
							ZipArchiveInputStream aZipInStream = new ZipArchiveInputStream(aPayload.getInputStream(), "utf-8", true, true);
				...
				{
					itsLog.error("Essential Viewer User File Service: Error encountered while receiving user payload.");
					theResponse.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		String aBearerToken = aNewNameMap.get(ViewerSecurityManager.USER_ACCOUNT_URL);
	...
	protected String getFolderJSON(File theFolder, String theRelativeFolderPath, HttpServletRequest theRequest)
	{
		...
		if(theFolder != null && theFolder.isDirectory())
		{
			...
			{
				itsLog.error("Attempting to build JSON string: {}", ex);
			}
		}
		return aJSON;
	}
	...
	protected synchronized boolean savePostedFile(FileItem theUncompressedItem, String theFileName, String theTargetFolder)
	{
		...
		{
			itsLog.error("Essential Viewer User File Service: Exception writing user upload uncompressed payload file: {}", ioEx);
			isSuccess = false;
		}		
	 ...
	 */
	protected synchronized boolean savePostedFile(ZipArchiveInputStream theSourceArchive, String theTargetFolder)
	{
		boolean isSuccess = false;
		...
		try
		{
			...
			ZipArchiveEntry aZipEntry = null;
			while((aZipEntry = theSourceArchive.getNextZipEntry()) != null)
			{
		...
		catch(IOException anXMLFileEx)
		{
			...
			itsLog.error("Error encountered while receiving user payload.: IO Exception while reading received Zip archive. Details: {}", anXMLFileEx);			
		}
		catch(NullPointerException anNPE)
		{
			itsLog.error("Error encountered while receiving user payload.: Exception reading received Zip archive. Details: {}", anNPE);			
		}
		catch(Exception anZipFileEx)
		{
			// Catch and report any other Exception reading update script file
			itsLog.error("Error encountered while receiving user payload.: Exception reading received Zip archive. Details: {}", anZipFileEx);			
		}
	...
	protected synchronized boolean saveNewFolder(String theNewFolderName, String theTargetFolder)
	{
		...
		if(theNewFolderName != null && !theNewFolderName.equals(""))
		{
			...
			{
				itsLog.error("Exception creating new folder: {}", ioEx);				
				isSuccess = false;
			}
		}
		else
		{
			itsLog.error("Essential Viewer User File Service: Attempt to create new folder with empty or null name.");
			isSuccess = false;
		}
