/**
 * Copyright (c)2013-2019 Enterprise Architecture Solutions Ltd.
...
public class VisitHistoryService extends HttpServlet 
{
	 ...
	 */
	private static final Logger itsLog = LoggerFactory.getLogger(VisitHistoryService.class);	
	...
	protected void doPost(HttpServletRequest theRequest, HttpServletResponse theResponse) throws ServletException, IOException
	{
		...
		if(itIsTrackingHistory)
		{
			...
			// Debug			
			itsLog.debug(">>> VisitHistoryService.doPost(). un-encoded URL = {}, uncoded Label = {}", aURL, aLabel);			
			...
			// Debug
			itsLog.debug(">>> Encoding...");
			itsLog.debug(">>> VisitHistoryService.doPost(). Encoded URL = {}, Encoded Label = {}", aURL, aLabel);
			itsLog.debug(">>> ");
		...
		{
			itsLog.error("VisitHistoryService Error processing configuration XML file: {}", aJaxbEx);
		}		
		catch (IllegalArgumentException anIllegalArgEx)
		{
			itsLog.error("VisitHistoryService Error unmarshalling configuration XML file: {}", anIllegalArgEx);			
		}
	}
